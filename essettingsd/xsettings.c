...
typedef struct _ExpidusXSettingsScreen ExpidusXSettingsScreen;
typedef struct _ExpidusXSetting        ExpidusXSetting;
typedef struct _ExpidusXSettingsNotify ExpidusXSettingsNotify;



static void     expidus_xsettings_helper_finalize     (GObject             *object);
static void     expidus_xsettings_helper_fc_free      (ExpidusXSettingsHelper *helper);
static gboolean expidus_xsettings_helper_fc_init      (gpointer             data);
static gboolean expidus_xsettings_helper_notify_idle  (gpointer             data);
static void     expidus_xsettings_helper_setting_free (gpointer             data);
static void     expidus_xsettings_helper_prop_changed (EsconfChannel       *channel,
                                                    const gchar         *prop_name,
                                                    const GValue        *value,
                                                    ExpidusXSettingsHelper *helper);
static void     expidus_xsettings_helper_load         (ExpidusXSettingsHelper *helper);
static void     expidus_xsettings_helper_screen_free  (ExpidusXSettingsScreen *screen);
static void     expidus_xsettings_helper_notify_xft   (ExpidusXSettingsHelper *helper);
static void     expidus_xsettings_helper_notify       (ExpidusXSettingsHelper *helper);



struct _ExpidusXSettingsHelperClass
{
    GObjectClass __parent__;
};

struct _ExpidusXSettingsHelper
{
    GObject  __parent__;

    EsconfChannel *channel;

    /* list of ExpidusXSettingsScreen we handle */
    GSList        *screens;

    /* table with esconf property keyd and ExpidusXSetting */
    GHashTable    *settings;
...
struct _ExpidusXSetting
{
    GValue *value;
    gulong  last_change_serial;
};

struct _ExpidusXSettingsNotify
{
    guchar *buf;
    gsize   buf_len;
    gint    n_settings;
...
struct _ExpidusXSettingsScreen
{
    Display *xdisplay;
    Window   window;
    Atom     selection_atom;
...
G_DEFINE_TYPE (ExpidusXSettingsHelper, expidus_xsettings_helper, G_TYPE_OBJECT);



static void
expidus_xsettings_helper_class_init (ExpidusXSettingsHelperClass *klass)
{
    GObjectClass *gobject_class;

    gobject_class = G_OBJECT_CLASS (klass);
    gobject_class->finalize = expidus_xsettings_helper_finalize;
}



static void
expidus_xsettings_helper_init (ExpidusXSettingsHelper *helper)
{
    helper->channel = esconf_channel_new ("xsettings");

    helper->settings = g_hash_table_new_full (g_str_hash, g_str_equal,
        g_free, expidus_xsettings_helper_setting_free);

    expidus_xsettings_helper_load (helper);

    g_signal_connect (G_OBJECT (helper->channel), "property-changed",
        G_CALLBACK (expidus_xsettings_helper_prop_changed), helper);
}



static void
expidus_xsettings_helper_finalize (GObject *object)
{
    ExpidusXSettingsHelper *helper = EXPIDUS_XSETTINGS_HELPER (object);
    GSList              *li;

    /* stop fontconfig monitoring */
    expidus_xsettings_helper_fc_free (helper);
    ...
    for (li = helper->screens; li != NULL; li = li->next)
        expidus_xsettings_helper_screen_free (li->data);
    g_slist_free (helper->screens);

    g_hash_table_destroy (helper->settings);

    (*G_OBJECT_CLASS (expidus_xsettings_helper_parent_class)->finalize) (object);
}



static gboolean
expidus_xsettings_helper_fc_notify (gpointer data)
{
    ExpidusXSettingsHelper *helper = EXPIDUS_XSETTINGS_HELPER (data);
    ExpidusXSetting        *setting;
    ...
    if (!FcConfigUptoDate (NULL) && FcInitReinitialize ())
    {
        ...
        {
            /* create new setting */
            setting = g_slice_new0 (ExpidusXSetting);
        ...
        essettings_dbg (XFSD_DEBUG_FONTCONFIG, "timestamp updated (time=%d)",
                        g_value_get_int (setting->value));

        /* schedule xsettings update */
        if (helper->notify_idle_id == 0)
            helper->notify_idle_id = g_idle_add (expidus_xsettings_helper_notify_idle, helper);

        /* restart monitoring */
        helper->fc_init_id = g_idle_add (expidus_xsettings_helper_fc_init, helper);
    }
...
static void
expidus_xsettings_helper_fc_changed (ExpidusXSettingsHelper *helper)
{
    /* reschedule monitor */
    ...
    helper->fc_notify_timeout_id = g_timeout_add_seconds (FC_TIMEOUT_SEC,
        expidus_xsettings_helper_fc_notify, helper);
}



static void
expidus_xsettings_helper_fc_free (ExpidusXSettingsHelper *helper)
{
    if (helper->fc_notify_timeout_id != 0)
    {
        /* stop update timeout */
...
static void
expidus_xsettings_helper_fc_monitor (ExpidusXSettingsHelper *helper,
                                  FcStrList           *files)
{
    const gchar  *path;
    GFile        *file;
    ...
    for (;;)
    {
        ...
        if (G_LIKELY (monitor != NULL))
        {
            ...
            g_signal_connect_swapped (G_OBJECT (monitor), "changed",
                G_CALLBACK (expidus_xsettings_helper_fc_changed), helper);

            essettings_dbg_filtered (XFSD_DEBUG_FONTCONFIG, "monitoring \"%s\"",
                                     path);
        }
    }
...
static gboolean
expidus_xsettings_helper_fc_init (gpointer data)
{
    ExpidusXSettingsHelper *helper = EXPIDUS_XSETTINGS_HELPER (data);
    ...
    if (FcInit ())
    {
        ...
        expidus_xsettings_helper_fc_monitor (helper, FcConfigGetConfigFiles (NULL));
        expidus_xsettings_helper_fc_monitor (helper, FcConfigGetFontDirs (NULL));

        essettings_dbg (XFSD_DEBUG_FONTCONFIG, "monitoring %d paths",
...
static gboolean
expidus_xsettings_helper_notify_idle (gpointer data)
{
    ExpidusXSettingsHelper *helper = EXPIDUS_XSETTINGS_HELPER (data);

    /* only update if there are screen registered */
    if (helper->screens != NULL)
        expidus_xsettings_helper_notify (helper);
...
static gboolean
expidus_xsettings_helper_notify_xft_idle (gpointer data)
{
    ExpidusXSettingsHelper *helper = EXPIDUS_XSETTINGS_HELPER (data);

    /* only update if there are screen registered */
    if (helper->screens != NULL)
        expidus_xsettings_helper_notify_xft (helper);
...
static gboolean
expidus_xsettings_helper_prop_valid (const gchar  *prop_name,
                                  const GValue *value)
{
...
static gboolean
expidus_xsettings_helper_prop_load (gchar               *prop_name,
                                 GValue              *value,
                                 ExpidusXSettingsHelper *helper)
{
    ExpidusXSetting *setting;
    ...
    if (!expidus_xsettings_helper_prop_valid (prop_name, value))
        return FALSE;
    ...
    essettings_dbg_filtered (XFSD_DEBUG_XSETTINGS, "prop \"%s\" loaded (type=%s)",
                             prop_name, G_VALUE_TYPE_NAME (value));
...
static void
expidus_xsettings_helper_prop_changed (EsconfChannel       *channel,
                                    const gchar         *prop_name,
                                    const GValue        *value,
                                    ExpidusXSettingsHelper *helper)
{
    ...
    essettings_dbg_filtered (XFSD_DEBUG_XSETTINGS, "prop \"%s\" changed (type=%s)",
                             prop_name, G_VALUE_TYPE_NAME (value));
    ...
    if (G_LIKELY (value != NULL))
    {
        ...
        if (G_LIKELY (setting != NULL))
        {
             ...
             * you cannot changes types in esconf without removing
             * it first */
        ...
        else if (expidus_xsettings_helper_prop_valid (prop_name, value))
        {
    ...
    {
        helper->notify_xft_idle_id = g_idle_add (expidus_xsettings_helper_notify_xft_idle, helper);
    }
}
...
static void
expidus_xsettings_helper_load (ExpidusXSettingsHelper *helper)
{
    GHashTable *props;

    props = esconf_channel_get_properties (helper->channel, NULL);
    if (G_LIKELY (props != NULL))
      {
        /* steal properties and put them in the settings table */
        g_hash_table_foreach_steal (props,
            (GHRFunc) expidus_xsettings_helper_prop_load, helper);
...
static void
expidus_xsettings_helper_setting_free (gpointer data)
{
    ExpidusXSetting *setting = data;

    g_value_unset (setting->value);
    g_free (setting->value);
    g_slice_free (ExpidusXSetting, setting);
}



static gint
expidus_xsettings_helper_screen_dpi (ExpidusXSettingsScreen *screen)
{
    Screen *xscreen;
    ...
    essettings_dbg_filtered (XFSD_DEBUG_XSETTINGS, "calculated dpi of %d for screen %d",
...
static void
expidus_xsettings_helper_notify_xft_update (GString      *resource,
                                         const gchar  *name,
                                         const GValue *value)
{
    gchar       *found;
...
static void
expidus_xsettings_helper_notify_xft (ExpidusXSettingsHelper *helper)
{
    Display      *xdisplay;
    gchar        *str;
    GString      *resource;
    ...
    g_return_if_fail (EXPIDUS_IS_XSETTINGS_HELPER (helper));
    ...
    for (i = 0; i < G_N_ELEMENTS (props); i++)
    {
        ...
        if (G_LIKELY (setting != NULL))
        {
            expidus_xsettings_helper_notify_xft_update (resource, props[i][1],
                                                     setting->value);
        }
    }
    ...
    expidus_xsettings_helper_notify_xft_update (resource, "Xcursor.theme_core:", &bool_val);
    ...
    essettings_dbg (XFSD_DEBUG_XSETTINGS,
...
static void
expidus_xsettings_helper_setting_append (const gchar         *name,
                                      ExpidusXSetting        *setting,
                                      ExpidusXSettingsNotify *notify)
{
    ...
    name_len = strlen (name) - 1 /* -1 for the esconf slash */;
    ...
    *(CARD16 *)needle = name_len;
    ...
    /* name */
    memcpy (needle, name + 1 /* +1 for the esconf slash */, name_len);
    needle += name_len;
...
static void
expidus_xsettings_helper_notify (ExpidusXSettingsHelper *helper)
{
    ExpidusXSettingsNotify *notify;
    CARD32               orderint = 0x01020304;
    guchar              *needle;
    ExpidusXSettingsScreen *screen;
    GSList              *li;
    gint                 dpi;

    g_return_if_fail (EXPIDUS_IS_XSETTINGS_HELPER (helper));

    notify = g_slice_new0 (ExpidusXSettingsNotify);
    ...
    *(CARD32 *)needle = helper->serial++;
    ...
    g_hash_table_foreach (helper->settings,
        (GHFunc) expidus_xsettings_helper_setting_append, notify);
    ...
    *(CARD32 *)needle = notify->n_settings;
    ...
    for (li = helper->screens; li != NULL; li = li->next)
    {
        ...
        {
            dpi = expidus_xsettings_helper_screen_dpi (screen);
    ...
    g_free (notify->buf);
  errnomem:
    g_slice_free (ExpidusXSettingsNotify, notify);
}



static void
expidus_xsettings_helper_screen_free (ExpidusXSettingsScreen *screen)
{
    XDestroyWindow (screen->xdisplay, screen->window);
    g_slice_free (ExpidusXSettingsScreen, screen);
}



static GdkFilterReturn
expidus_xsettings_helper_event_filter (GdkXEvent *gdkxevent,
                                    GdkEvent  *gdkevent,
                                    gpointer   data)
{
    ExpidusXSettingsHelper *helper = EXPIDUS_XSETTINGS_HELPER (data);
    ...
    if (xevent->xany.type == SelectionClear)
    {
        for (li = helper->screens; li != NULL; li = li->next)
        {
                ...
                && xevent->xselectionclear.selection == screen->selection_atom)
            {
                ...
                expidus_xsettings_helper_screen_free (screen);

                essettings_dbg (XFSD_DEBUG_XSETTINGS, "lost selection, %d screens left",
                                g_slist_length (helper->screens));

                /* remove this filter if there are no screens */
                if (helper->screens == NULL)
                    gdk_window_remove_filter (NULL, expidus_xsettings_helper_event_filter, data);
...
static Bool
expidus_xsettings_helper_timestamp_predicate (Display  *xdisplay,
                                           XEvent   *xevent,
                                           XPointer  arg)
{
...
Time
expidus_xsettings_get_server_time (Display *xdisplay,
                                Window   window)
{
    Atom   timestamp_atom;
    guchar c = 'a';
    ...
    XIfEvent (xdisplay, &xevent, expidus_xsettings_helper_timestamp_predicate,
...
gboolean
expidus_xsettings_helper_register (ExpidusXSettingsHelper *helper,
                                ...
                                gboolean             force_replace)
{
    ...
    g_return_val_if_fail (EXPIDUS_IS_XSETTINGS_HELPER (helper), FALSE);
    ...
    for (n = 0; n < n_screens; n++)
    {
        ...
        /* get the current xserver timestamp */
        timestamp = expidus_xsettings_get_server_time (xdisplay, window);
        ...
        if (G_LIKELY (succeed))
        {
            ...
            screen = g_slice_new0 (ExpidusXSettingsScreen);
            screen->window = window;
            screen->selection_atom = selection_atom;
            screen->xdisplay = xdisplay;
            screen->screen_num = n;

            essettings_dbg (XFSD_DEBUG_XSETTINGS, "%s registered on screen %d", atom_name, n);
    ...
    if (helper->screens != NULL)
    {
        ...
        gdk_window_add_filter (NULL, expidus_xsettings_helper_event_filter, helper);
