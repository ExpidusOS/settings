...
typedef struct _ExpidusRRCrtc   ExpidusRRCrtc;
typedef struct _ExpidusRROutput ExpidusRROutput;



static void             expidus_displays_helper_dispose                        (GObject                 *object);
static void             expidus_displays_helper_finalize                       (GObject                 *object);
static void             expidus_displays_helper_reload                         (ExpidusDisplaysHelper      *helper);
static gchar           *expidus_displays_helper_get_matching_profile           (ExpidusDisplaysHelper      *helper);
static GdkFilterReturn  expidus_displays_helper_screen_on_event                (GdkXEvent               *xevent,
                                                                             GdkEvent                *event,
                                                                             gpointer                 data);
static void             expidus_displays_helper_set_screen_size                (ExpidusDisplaysHelper      *helper);
static gboolean         expidus_displays_helper_load_from_esconf               (ExpidusDisplaysHelper      *helper,
                                                                             const gchar             *scheme,
                                                                             GHashTable              *saved_outputs,
                                                                             ExpidusRROutput            *output);
static GPtrArray       *expidus_displays_helper_list_outputs                   (ExpidusDisplaysHelper      *helper);
static void             expidus_displays_helper_free_output                    (ExpidusRROutput            *output);
static GPtrArray       *expidus_displays_helper_list_crtcs                     (ExpidusDisplaysHelper      *helper);
static ExpidusRRCrtc      *expidus_displays_helper_find_crtc_by_id                (ExpidusDisplaysHelper      *helper,
                                                                             RRCrtc                   id);
static void             expidus_displays_helper_free_crtc                      (ExpidusRRCrtc              *crtc);
static ExpidusRRCrtc      *expidus_displays_helper_find_usable_crtc               (ExpidusDisplaysHelper      *helper,
                                                                             ExpidusRROutput            *output);
static void             expidus_displays_helper_get_topleftmost_pos            (ExpidusRRCrtc              *crtc,
                                                                             ExpidusDisplaysHelper      *helper);
static void             expidus_displays_helper_normalize_crtc                 (ExpidusRRCrtc              *crtc,
                                                                             ExpidusDisplaysHelper      *helper);
static Status           expidus_displays_helper_disable_crtc                   (ExpidusDisplaysHelper      *helper,
                                                                             RRCrtc                   crtc);
static void             expidus_displays_helper_workaround_crtc_size           (ExpidusRRCrtc              *crtc,
                                                                             ExpidusDisplaysHelper      *helper);
static void             expidus_displays_helper_apply_crtc_transform           (ExpidusRRCrtc              *crtc,
                                                                             ExpidusDisplaysHelper      *helper);
static void             expidus_displays_helper_apply_crtc                     (ExpidusRRCrtc              *crtc,
                                                                             ExpidusDisplaysHelper      *helper);
static void             expidus_displays_helper_set_outputs                    (ExpidusRRCrtc              *crtc,
                                                                             ExpidusRROutput            *output);
static void             expidus_displays_helper_apply_all                      (ExpidusDisplaysHelper      *helper);
static void             expidus_displays_helper_channel_apply                  (ExpidusDisplaysHelper      *helper,
                                                                             const gchar             *scheme);
static void             expidus_displays_helper_channel_property_changed       (EsconfChannel           *channel,
                                                                             const gchar             *property_name,
                                                                             const GValue            *value,
                                                                             ExpidusDisplaysHelper      *helper);
static void             expidus_displays_helper_toggle_internal                (gpointer                *power,
                                                                             gboolean                 lid_is_closed,
                                                                             ExpidusDisplaysHelper      *helper);



struct _ExpidusDisplaysHelperClass
{
    GObjectClass __parent__;
};

struct _ExpidusDisplaysHelper
{
    GObject  __parent__;
...
G_DEFINE_TYPE (ExpidusDisplaysHelper, expidus_displays_helper, G_TYPE_OBJECT);



static void
expidus_displays_helper_class_init (ExpidusDisplaysHelperClass *klass)
{
    GObjectClass *gobject_class = G_OBJECT_CLASS (klass);

    gobject_class->dispose = expidus_displays_helper_dispose;
    gobject_class->finalize = expidus_displays_helper_finalize;
}



static void
expidus_displays_helper_init (ExpidusDisplaysHelper *helper)
{
    ...
    if (XRRQueryExtension (helper->xdisplay, &helper->event_base, &error_base))
    {
            ...
            && (major > 1 || (major == 1 && minor >= 2)))
        {
            ...
            helper->crtcs = expidus_displays_helper_list_crtcs (helper);
            helper->outputs = expidus_displays_helper_list_outputs (helper);
            ...
            gdk_window_add_filter (helper->root_window,
                                   expidus_displays_helper_screen_on_event,
                                   helper);

#ifdef HAVE_UPOWERGLIB
            helper->power = g_object_new (EXPIDUS_TYPE_DISPLAYS_UPOWER, NULL);
            helper->phandler = g_signal_connect (G_OBJECT (helper->power),
                                                 "lid-changed",
                                                 G_CALLBACK (expidus_displays_helper_toggle_internal),
                                                 helper);
#endif

            /* open the channel */
            helper->channel = esconf_channel_get ("displays");

            /* remove any leftover apply property before setting the monitor */
            esconf_channel_reset_property (helper->channel, APPLY_SCHEME_PROP, FALSE);
            esconf_channel_set_string (helper->channel, ACTIVE_PROFILE, DEFAULT_SCHEME_NAME);

            /* monitor channel changes */
            helper->handler = g_signal_connect (G_OBJECT (helper->channel),
                                                "property-changed",
                                                G_CALLBACK (expidus_displays_helper_channel_property_changed),
                                                helper);
            ...
            if (esconf_channel_get_bool (helper->channel, AUTO_ENABLE_PROFILES, FALSE) &&
                esconf_channel_get_bool (helper->channel, NOTIFY_PROP, FALSE))
            {
                gchar *matching_profile = NULL;

                matching_profile = expidus_displays_helper_get_matching_profile (helper);
                if (matching_profile)
                {
                    expidus_displays_helper_channel_apply (helper, matching_profile);
                }
                else {
                    expidus_displays_helper_channel_apply (helper, DEFAULT_SCHEME_NAME);
                }
            }
...
static void
expidus_displays_helper_dispose (GObject *object)
{
    ExpidusDisplaysHelper *helper = EXPIDUS_DISPLAYS_HELPER (object);
    ...
    (*G_OBJECT_CLASS (expidus_displays_helper_parent_class)->dispose) (object);
}



static void
expidus_displays_helper_finalize (GObject *object)
{
    ExpidusDisplaysHelper *helper = EXPIDUS_DISPLAYS_HELPER (object);
    ...
    (*G_OBJECT_CLASS (expidus_displays_helper_parent_class)->finalize) (object);
}



static void
expidus_displays_helper_reload (ExpidusDisplaysHelper *helper)
{
    gint err;

    essettings_dbg (XFSD_DEBUG_DISPLAYS, "Refreshing RandR cache.");
    ...
    /* get the screen resource */
#ifdef HAS_RANDR_ONE_POINT_THREE
    /* expidus_displays_helper_reload () is usually called after a xrandr notification,
...
static gchar **
expidus_displays_helper_get_display_infos (gint       noutput,
                                        Display   *xdisplay,
                                        GPtrArray *outputs)
{
    gchar    **display_infos;
    ...
    for (m = 0; m < noutput; ++m)
    {
        ...
        edid_data = expidus_randr_read_edid_data (xdisplay, output->id);
...
static gchar *
expidus_displays_helper_get_matching_profile (ExpidusDisplaysHelper *helper)
{
    GList              *profiles = NULL;
    gpointer           *profile;
    gchar              *profile_name;
    ...
    display_infos = expidus_displays_helper_get_display_infos (helper->outputs->len,
                                                            helper->xdisplay,
                                                            helper->outputs);
    if (display_infos)
    {
    ...
    if (profiles == NULL)
    {
        essettings_dbg (XFSD_DEBUG_DISPLAYS, "No matching display profiles found.");
    }
    else if (g_list_length (profiles) == 1)
    {
        profile = g_list_nth_data (profiles, 0);
        property = g_strdup_printf ("/%s", (gchar *) profile);
        profile_name = esconf_channel_get_string (helper->channel, property, NULL);
        essettings_dbg (XFSD_DEBUG_DISPLAYS, "Applied the only matching display profile: %s", profile_name);
        g_free (profile_name);
        g_free (property);
    ...
    {
        essettings_dbg (XFSD_DEBUG_DISPLAYS, "Found %d matching display profiles.", g_list_length (profiles));
    }
...
expidus_displays_helper_screen_on_event (GdkXEvent *xevent,
                                      GdkEvent  *event,
                                      gpointer   data)
{
    ExpidusDisplaysHelper *helper = EXPIDUS_DISPLAYS_HELPER (data);
    GPtrArray          *old_outputs;
    ExpidusRRCrtc         *crtc = NULL;
    ExpidusRROutput       *output, *o;
    XEvent             *e = xevent;
    gint                event_num;
    gint                j;
    guint               n, m, nactive = 0;
    ...
    {
        essettings_dbg (XFSD_DEBUG_DISPLAYS, "RRScreenChangeNotify event received.");

        old_outputs = g_ptr_array_ref (helper->outputs);
        expidus_displays_helper_reload (helper);

        essettings_dbg (XFSD_DEBUG_DISPLAYS, "Noutput: before = %d, after = %d.",
        ...
        if (old_outputs->len > helper->outputs->len)
        {
            ...
            for (n = 0; n < old_outputs->len; ++n)
            {
                ...
                if (!found)
                {
                    essettings_dbg (XFSD_DEBUG_DISPLAYS, "Output disconnected: %s",
                                    output->info->name);
                    /* force deconfiguring the crtc for the removed output */
                    if (output->info->crtc != None)
                        crtc = expidus_displays_helper_find_crtc_by_id (helper,
                                                                     output->info->crtc);
                    if (crtc)
                    {
                        crtc->mode = None;
                        expidus_displays_helper_disable_crtc (helper, crtc->id);
                    }
            ...
            if (nactive == 0)
            {
                essettings_dbg (XFSD_DEBUG_DISPLAYS, "No active output anymore! "
                                "Attempting to re-enable the internal output.");
                expidus_displays_helper_toggle_internal (NULL, FALSE, helper);
            }
            else if (changed)
                expidus_displays_helper_apply_all (helper);
        }
        else
        {
            ...
            for (n = 0; n < helper->outputs->len; ++n)
            {
                ...
                if (!found)
                {
                    essettings_dbg (XFSD_DEBUG_DISPLAYS, "New output connected: %s",
                                    output->info->name);
                    /* need to enable crtc for output ? */
                    if (output->info->crtc == None)
                    {
                        essettings_dbg (XFSD_DEBUG_DISPLAYS, "enabling crtc for %s", output->info->name);
                        crtc = expidus_displays_helper_find_usable_crtc (helper, output);
                        if (crtc)
                        {
...
G_GNUC_END_IGNORE_DEPRECATIONS
                            ...
                            expidus_displays_helper_set_outputs (crtc, output);
                            crtc->changed = TRUE;
                        }
                    }
            ...
            if (changed && esconf_channel_get_bool (helper->channel, NOTIFY_PROP, FALSE))
                expidus_spawn_command_line_on_screen (NULL, "expidus1-display-settings -m", FALSE,
                                                   FALSE, NULL);
        }
...
static void
expidus_displays_helper_set_screen_size (ExpidusDisplaysHelper *helper)
{
    gint min_width, min_height, max_width, max_height;

    g_assert (EXPIDUS_IS_DISPLAYS_HELPER (helper) && helper->xdisplay && helper->resources);
...
G_GNUC_BEGIN_IGNORE_DEPRECATIONS
    essettings_dbg (XFSD_DEBUG_DISPLAYS, "min_h = %d, min_w = %d, max_h = %d, max_w = %d, "
    ...
    {
        essettings_dbg (XFSD_DEBUG_DISPLAYS, "Applying desktop dimensions: %dx%d (px), %dx%d (mm).",
...
static gboolean
expidus_displays_helper_load_from_esconf (ExpidusDisplaysHelper *helper,
                                       const gchar        *scheme,
                                       GHashTable         *saved_outputs,
                                       ExpidusRROutput       *output)
{
    ...
    g_assert (EXPIDUS_IS_DISPLAYS_HELPER (helper) && helper->resources && output);

    active = output->active;

    /* does this output exist in esconf? */
    ...
    if (!g_value_get_boolean (value))
    {
        ...
        {
            active = FALSE;
            essettings_dbg (XFSD_DEBUG_DISPLAYS, "%s will be disabled by configuration.",
                            output->info->name);
...
static GPtrArray *
expidus_displays_helper_list_outputs (ExpidusDisplaysHelper *helper)
{
    GPtrArray     *outputs;
    ...
    outputs = g_ptr_array_new_with_free_func ((GDestroyNotify) expidus_displays_helper_free_output);
    for (n = 0; n < helper->resources->noutput; ++n)
    {
        ...
        output = g_new0 (ExpidusRROutput, 1);
...
G_GNUC_END_IGNORE_DEPRECATIONS
        ...
        crtc = expidus_displays_helper_find_crtc_by_id (helper, output->info->crtc);
        output->active = crtc && crtc->mode != None;

        /* Translate output->name into esconf compatible format in place */
        g_strcanon(output->info->name, "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-_<>", '_');

        essettings_dbg (XFSD_DEBUG_DISPLAYS, "Detected output %lu %s.", output->id,
                        output->info->name);
...
static void
expidus_displays_helper_free_output (ExpidusRROutput *output)
{
    if (output == NULL)
        return;
...
static GPtrArray *
expidus_displays_helper_list_crtcs (ExpidusDisplaysHelper *helper)
{
    GPtrArray   *crtcs;
    ...
    crtcs = g_ptr_array_new_with_free_func ((GDestroyNotify) expidus_displays_helper_free_crtc);
    for (n = 0; n < helper->resources->ncrtc; ++n)
    {
        XRRCrtcTransformAttributes  *attr;
        essettings_dbg (XFSD_DEBUG_DISPLAYS, "Detected CRTC %lu.", helper->resources->crtcs[n]);
...
static ExpidusRRCrtc *
expidus_displays_helper_find_crtc_by_id (ExpidusDisplaysHelper *helper,
                                      RRCrtc              id)
{
    ExpidusRRCrtc *crtc;
    guint       n;

    g_assert (EXPIDUS_IS_DISPLAYS_HELPER (helper) && helper->crtcs);
...
static void
expidus_displays_helper_free_crtc (ExpidusRRCrtc *crtc)
{
    if (crtc == NULL)
        return;
...
static ExpidusRRCrtc *
expidus_displays_helper_find_usable_crtc (ExpidusDisplaysHelper *helper,
                                       ExpidusRROutput       *output)
{
    ExpidusRRCrtc *crtc = NULL;
    guint       n;
    ...
    g_assert (EXPIDUS_IS_DISPLAYS_HELPER (helper) && helper->crtcs && output);
    ...
    if (found)
        essettings_dbg (XFSD_DEBUG_DISPLAYS, "CRTC %lu assigned to %s.", crtc->id,
                        output->info->name);
    else
...
static void
expidus_displays_helper_get_topleftmost_pos (ExpidusRRCrtc         *crtc,
                                          ExpidusDisplaysHelper *helper)
{
    g_assert (EXPIDUS_IS_DISPLAYS_HELPER (helper) && crtc);
...
static void
expidus_displays_helper_normalize_crtc (ExpidusRRCrtc         *crtc,
                                     ExpidusDisplaysHelper *helper)
{
    g_assert (EXPIDUS_IS_DISPLAYS_HELPER (helper) && crtc);
    ...
    essettings_dbg (XFSD_DEBUG_DISPLAYS, "Normalized CRTC %lu: size=%dx%d, pos=%dx%d.",
...
static Status
expidus_displays_helper_disable_crtc (ExpidusDisplaysHelper *helper,
                                   RRCrtc              crtc)
{
    g_assert (EXPIDUS_IS_DISPLAYS_HELPER (helper) && helper->xdisplay && helper->resources);

    essettings_dbg (XFSD_DEBUG_DISPLAYS, "Disabling CRTC %lu.", crtc);
...
static void
expidus_displays_helper_workaround_crtc_size (ExpidusRRCrtc         *crtc,
                                           ExpidusDisplaysHelper *helper)
{
    XRRCrtcInfo *crtc_info;

    g_assert (EXPIDUS_IS_DISPLAYS_HELPER (helper) && helper->xdisplay && helper->resources && crtc);
    ...
    {
        essettings_dbg (XFSD_DEBUG_DISPLAYS, "CRTC %lu must be temporarily disabled.", crtc->id);
        if (expidus_displays_helper_disable_crtc (helper, crtc->id) == RRSetConfigSuccess)
...
static void
expidus_displays_helper_apply_crtc_transform (ExpidusRRCrtc         *crtc,
                                           ExpidusDisplaysHelper *helper)
{
    XTransform transform;
    gchar *filter;

    g_assert (EXPIDUS_IS_DISPLAYS_HELPER (helper) && helper->xdisplay && crtc);
    ...
    if (helper->has_1_3)
    {
        ...
        essettings_dbg (XFSD_DEBUG_DISPLAYS, "Applying CRTC %lu Transform: x=%lf y=%lf, filter=%s.", crtc->id,
...
static void
expidus_displays_helper_apply_crtc (ExpidusRRCrtc         *crtc,
                                 ExpidusDisplaysHelper *helper)
{
    Status ret;
    ...
    essettings_dbg (XFSD_DEBUG_DISPLAYS, "Configuring CRTC %lu.", crtc->id);

    /* check if we really need to do something */
    if (crtc->changed)
    {
        essettings_dbg (XFSD_DEBUG_DISPLAYS, "Applying changes to CRTC %lu.", crtc->id);

        if (crtc->mode == None) {
            ret = expidus_displays_helper_disable_crtc (helper, crtc->id);
        } else {
            expidus_displays_helper_apply_crtc_transform (crtc, helper);
...
static void
expidus_displays_helper_set_outputs (ExpidusRRCrtc   *crtc,
                                  ExpidusRROutput *output)
{
    gint n;
    ...
    for (n = 0; n < crtc->noutput; ++n)
        essettings_dbg (XFSD_DEBUG_DISPLAYS, "CRTC %lu, output list[%d] -> %lu.", crtc->id, n,
                        crtc->outputs[n]);
    ...
    essettings_dbg (XFSD_DEBUG_DISPLAYS, "CRTC %lu, output list[%d] -> %lu.", crtc->id,
...
static void
expidus_displays_helper_apply_all (ExpidusDisplaysHelper *helper)
{
    g_assert (EXPIDUS_IS_DISPLAYS_HELPER (helper) && helper->crtcs);
    ...
    g_ptr_array_foreach (helper->crtcs, (GFunc) expidus_displays_helper_get_topleftmost_pos, helper);
    g_ptr_array_foreach (helper->crtcs, (GFunc) expidus_displays_helper_normalize_crtc, helper);
    ...
    g_ptr_array_foreach (helper->crtcs, (GFunc) expidus_displays_helper_workaround_crtc_size, helper);

    /* set the screen size only if it's really needed and valid */
    expidus_displays_helper_set_screen_size (helper);

    /* final loop, apply crtc changes */
    g_ptr_array_foreach (helper->crtcs, (GFunc) expidus_displays_helper_apply_crtc, helper);
...
static void
expidus_displays_helper_channel_apply (ExpidusDisplaysHelper *helper,
                                    const gchar        *scheme)
{
    gchar       property[512];
    guint       n, nactive;
    ...
    esconf_channel_set_string (helper->channel, ACTIVE_PROFILE, scheme);

    /* finally the list of saved outputs from esconf */
    g_snprintf (property, sizeof (property), "/%s", scheme);
    saved_outputs = esconf_channel_get_properties (helper->channel, property);
    ...
    for (n = 0; n < helper->outputs->len; ++n)
    {
        if (expidus_displays_helper_load_from_esconf (helper, scheme, saved_outputs,
                                                   g_ptr_array_index (helper->outputs,
                                                                      n)))
            ++nactive;
    }

    essettings_dbg (XFSD_DEBUG_DISPLAYS, "Total %d active output(s).", nactive);

    /* safety check */
    if (nactive == 0)
    {
        g_critical ("Stored Esconf properties disable all outputs, aborting.");
        goto err_cleanup;
    }
...
static void
expidus_displays_helper_channel_property_changed (EsconfChannel      *channel,
                                               const gchar        *property_name,
                                               const GValue       *value,
                                               ExpidusDisplaysHelper *helper)
{
    ...
    {
        /* apply */
        expidus_displays_helper_channel_apply (helper, g_value_get_string (value));
        /* remove the apply property */
        esconf_channel_reset_property (channel, APPLY_SCHEME_PROP, FALSE);
    }
}
...
static void
expidus_displays_helper_toggle_internal (gpointer           *power,
                                      gboolean            lid_is_closed,
                                      ExpidusDisplaysHelper *helper)
{
    GHashTable    *saved_outputs;
    ExpidusRRCrtc    *crtc = NULL;
    ExpidusRROutput  *output, *lvds = NULL;
    ...
    essettings_dbg (XFSD_DEBUG_DISPLAYS, "Toggling internal output %s.",
                    lvds->info->name);

    if (lvds->active && lid_is_closed)
    {
        /* if active and the lid is closed, deactivate it */
        crtc = expidus_displays_helper_find_usable_crtc (helper, lvds);
        if (!crtc)
            return;
        crtc->mode = None;
        crtc->noutput = 0;
        crtc->changed = TRUE;
        essettings_dbg (XFSD_DEBUG_DISPLAYS, "%s will be disabled.", lvds->info->name);
    }
    else if (!lvds->active && !lid_is_closed)
    {
        /* re-activate it because the user opened the lid */
        saved_outputs = esconf_channel_get_properties (helper->channel, "/" DEFAULT_SCHEME_NAME);
        if (saved_outputs)
        {
            ...
            for (n = 0; n < helper->outputs->len; ++n)
            {
                ...
                expidus_displays_helper_load_from_esconf (helper, DEFAULT_SCHEME_NAME,
                                                       saved_outputs, output);
            }

            /* try to load user saved settings for lvds */
            active = expidus_displays_helper_load_from_esconf (helper, DEFAULT_SCHEME_NAME,
                                                            saved_outputs, lvds);
            g_hash_table_destroy (saved_outputs);
        }
        if (!active)
...
G_GNUC_END_IGNORE_DEPRECATIONS
            ...
            expidus_displays_helper_set_outputs (crtc, lvds);
            crtc->changed = TRUE;
        }
        essettings_dbg (XFSD_DEBUG_DISPLAYS, "%s will be re-enabled.",
                        lvds->info->name);
    }
    else
        return;
