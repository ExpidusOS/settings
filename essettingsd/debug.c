...
essettings_dbg_init (void)
{
    ...
    if (!inited)
    {
        value = g_getenv ("ESSETTINGSD_DEBUG");
...
essettings_dbg_print (XfsdDebugDomain  domain,
                      const gchar     *message,
                      va_list          args)
{
...
void
essettings_dbg (XfsdDebugDomain  domain,
                const gchar     *message,
                ...)
{
    va_list args;
    ...
    if (essettings_dbg_init () == 0)
        return;

    va_start (args, message);
    essettings_dbg_print (domain, message, args);
    va_end (args);
}
...
void
essettings_dbg_filtered (XfsdDebugDomain  domain,
                         const gchar     *message,
                         ...)
{
    va_list args;
    ...
    if ((essettings_dbg_init () & domain) == 0)
        return;
