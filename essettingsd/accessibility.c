...
static void            expidus_accessibility_helper_finalize                       (GObject                      *object);
static void            expidus_accessibility_helper_set_xkb                        (ExpidusAccessibilityHelper      *helper,
                                                                                 gulong                        mask);
static void            expidus_accessibility_helper_channel_property_changed       (EsconfChannel                *channel,
                                                                                 const gchar                  *property_name,
                                                                                 const GValue                 *value,
                                                                                 ExpidusAccessibilityHelper      *helper);
#ifdef HAVE_LIBNOTIFY
static GdkFilterReturn expidus_accessibility_helper_event_filter                   (GdkXEvent                    *xevent,
                                                                                 GdkEvent                     *gdk_event,
                                                                                 gpointer                      user_data);
static void            expidus_accessibility_helper_notification_closed            (NotifyNotification           *notification,
                                                                                 ExpidusAccessibilityHelper      *helper);
static void            expidus_accessibility_helper_notification_show              (ExpidusAccessibilityHelper      *helper,
                                                                                 const gchar                  *summary,
                                                                                 const gchar                  *body);
#endif /* !HAVE_LIBNOTIFY */
...
struct _ExpidusAccessibilityHelperClass
{
    GObjectClass __parent__;
};

struct _ExpidusAccessibilityHelper
{
    GObject  __parent__;
...
G_DEFINE_TYPE (ExpidusAccessibilityHelper, expidus_accessibility_helper, G_TYPE_OBJECT);



static void
expidus_accessibility_helper_class_init (ExpidusAccessibilityHelperClass *klass)
{
    GObjectClass *gobject_class;

    gobject_class = G_OBJECT_CLASS (klass);
    gobject_class->finalize = expidus_accessibility_helper_finalize;
}



static void
expidus_accessibility_helper_init (ExpidusAccessibilityHelper *helper)
{
    gint dummy;
    ...
    {
        /* open the channel */
        helper->channel = esconf_channel_get ("accessibility");

        /* monitor channel changes */
        g_signal_connect (G_OBJECT (helper->channel), "property-changed", G_CALLBACK (expidus_accessibility_helper_channel_property_changed), helper);

        /* restore the xbd configuration */
        expidus_accessibility_helper_set_xkb (helper, XkbStickyKeysMask | XkbSlowKeysMask | XkbBounceKeysMask | XkbMouseKeysMask | XkbAccessXKeysMask);

#ifdef HAVE_LIBNOTIFY
        /* setup a connection with the notification daemon */
        if (!notify_init ("expidus1-settings-helper"))
        ...
        gdk_window_add_filter (NULL, expidus_accessibility_helper_event_filter, helper);
#endif /* !HAVE_LIBNOTIFY */
    }
    else
    {
...
static void
expidus_accessibility_helper_finalize (GObject *object)
{
#ifdef HAVE_LIBNOTIFY
    ExpidusAccessibilityHelper *helper = EXPIDUS_ACCESSIBILITY_HELPER (object);
    ...
    (*G_OBJECT_CLASS (expidus_accessibility_helper_parent_class)->finalize) (object);
}



static void
expidus_accessibility_helper_set_xkb (ExpidusAccessibilityHelper *helper,
                                   gulong                   mask)
{
    ...
    if (G_LIKELY (xkb))
    {
        ...
        {
            if (esconf_channel_get_bool (helper->channel, "/AccessXKeys", FALSE))
            {
                ...
                essettings_dbg (XFSD_DEBUG_ACCESSIBILITY, "AccessXKeys enabled");
            }
            else
            {
                ...
                essettings_dbg (XFSD_DEBUG_ACCESSIBILITY, "AccessXKeys disabled");
            }
        }
        ...
        if (HAS_FLAG (mask, XkbStickyKeysMask))
        {
            if (esconf_channel_get_bool (helper->channel, "/StickyKeys", FALSE))
            {
                ...
                if (esconf_channel_get_bool (helper->channel, "/StickyKeys/LatchToLock", FALSE))
                    SET_FLAG (xkb->ctrls->ax_options, XkbAX_LatchToLockMask);
                else
                    UNSET_FLAG (xkb->ctrls->ax_options, XkbAX_LatchToLockMask);

                if (esconf_channel_get_bool (helper->channel, "/StickyKeys/TwoKeysDisable", FALSE))
                    SET_FLAG (xkb->ctrls->ax_options, XkbAX_TwoKeysMask);
                else
                    UNSET_FLAG (xkb->ctrls->ax_options, XkbAX_TwoKeysMask);

                essettings_dbg (XFSD_DEBUG_ACCESSIBILITY, "stickykeys enabled (ax_options=%d)",
            ...
            else
            {
                ...
                essettings_dbg (XFSD_DEBUG_ACCESSIBILITY, "stickykeys disabled");
            }
        }
        ...
        {
            if (esconf_channel_get_bool (helper->channel, "/SlowKeys", FALSE))
            {
                ...
                delay = esconf_channel_get_int (helper->channel, "/SlowKeys/Delay", 100);
                xkb->ctrls->slow_keys_delay = CLAMP (delay, 1, G_MAXUSHORT);

                essettings_dbg (XFSD_DEBUG_ACCESSIBILITY, "slowkeys enabled (delay=%d)",
            ...
            else
            {
                ...
                essettings_dbg (XFSD_DEBUG_ACCESSIBILITY, "slowkeys disabled");
            }
        }
        ...
        {
            if (esconf_channel_get_bool (helper->channel, "/BounceKeys", FALSE))
            {
                ...
                delay = esconf_channel_get_int (helper->channel, "/BounceKeys/Delay", 100);
                xkb->ctrls->debounce_delay = CLAMP (delay, 1, G_MAXUSHORT);

                essettings_dbg (XFSD_DEBUG_ACCESSIBILITY, "bouncekeys enabled (delay=%d)",
            ...
            else
            {
                ...
                essettings_dbg (XFSD_DEBUG_ACCESSIBILITY, "bouncekeys disabled");
            }
        }
        ...
        if (HAS_FLAG (mask, XkbMouseKeysMask))
        {
            if (esconf_channel_get_bool (helper->channel, "/MouseKeys", FALSE))
            {
                ...
                /* get values */
                delay = esconf_channel_get_int (helper->channel, "/MouseKeys/Delay", 160);
                interval = esconf_channel_get_int (helper->channel, "/MouseKeys/Interval", 20);
                time_to_max = esconf_channel_get_int (helper->channel, "/MouseKeys/TimeToMax", 3000);
                max_speed = esconf_channel_get_int (helper->channel, "/MouseKeys/MaxSpeed", 1000);
                curve = esconf_channel_get_int (helper->channel, "/MouseKeys/Curve", 0);
                ...
                essettings_dbg (XFSD_DEBUG_ACCESSIBILITY, "mousekeys enabled (delay=%d, interval=%d, "
            ...
            else
            {
                ...
                essettings_dbg (XFSD_DEBUG_ACCESSIBILITY, "mousekeys disabled");
            }
        }
...
static void
expidus_accessibility_helper_channel_property_changed (EsconfChannel           *channel,
                                                    const gchar             *property_name,
                                                    const GValue            *value,
                                                    ExpidusAccessibilityHelper *helper)
{
    ...
    expidus_accessibility_helper_set_xkb (helper, mask);
}


#ifdef HAVE_LIBNOTIFY
static GdkFilterReturn
expidus_accessibility_helper_event_filter (GdkXEvent *xevent,
                                        GdkEvent  *gdk_event,
                                        gpointer   user_data)
{
    XkbEvent                *event = xevent;
    ExpidusAccessibilityHelper *helper = EXPIDUS_ACCESSIBILITY_HELPER (user_data);
    const gchar             *body;
    ...
    switch (event->any.xkb_type)
    {
        case XkbControlsNotify:
            if (HAS_FLAG (event->ctrls.enabled_ctrl_changes, XkbStickyKeysMask))
            {
                ...
                expidus_accessibility_helper_notification_show (helper, _("Sticky keys"), body);
            }
            else if (HAS_FLAG (event->ctrls.enabled_ctrl_changes, XkbSlowKeysMask))
            {
                ...
                expidus_accessibility_helper_notification_show (helper, _("Slow keys"), body);
            }
            else if (HAS_FLAG (event->ctrls.enabled_ctrl_changes, XkbBounceKeysMask))
            {
                ...
                expidus_accessibility_helper_notification_show (helper, _("Bounce keys"), body);
            }
...
static void
expidus_accessibility_helper_notification_closed (NotifyNotification      *notification,
                                               ExpidusAccessibilityHelper *helper)
{
...
static void
expidus_accessibility_helper_notification_show (ExpidusAccessibilityHelper *helper,
                                             const gchar             *summary,
                                             const gchar             *body)
{
    ...
    if (helper->notification == NULL)
    {
        ...
        /* close signal */
        g_signal_connect (G_OBJECT (helper->notification), "closed", G_CALLBACK (expidus_accessibility_helper_notification_closed), helper);
    }
    else
    {
