...
static void             expidus_pointers_helper_finalize                 (GObject            *object);
static void             expidus_pointers_helper_syndaemon_stop           (ExpidusPointersHelper *helper);
static void             expidus_pointers_helper_syndaemon_check          (ExpidusPointersHelper *helper);
static void             expidus_pointers_helper_restore_devices          (ExpidusPointersHelper *helper,
                                                                       XID                *xid);
static void             expidus_pointers_helper_channel_property_changed (EsconfChannel      *channel,
                                                                       const gchar        *property_name,
                                                                       const GValue       *value,
                                                                       ExpidusPointersHelper *helper);
#ifdef DEVICE_HOTPLUGGING
static GdkFilterReturn  expidus_pointers_helper_event_filter             (GdkXEvent          *xevent,
                                                                       GdkEvent           *gdk_event,
                                                                       gpointer            user_data);
#endif
#if defined(DEVICE_PROPERTIES) || defined(HAVE_LIBINPUT)
static void             expidus_pointers_helper_change_property          (XDeviceInfo        *device_info,
                                                                       XDevice            *device,
                                                                       Display            *xdisplay,
...
struct _ExpidusPointersHelperClass
{
    GObjectClass __parent__;
};

struct _ExpidusPointersHelper
{
    GObject  __parent__;
...
G_DEFINE_TYPE (ExpidusPointersHelper, expidus_pointers_helper, G_TYPE_OBJECT);



static void
expidus_pointers_helper_class_init (ExpidusPointersHelperClass *klass)
{
    GObjectClass *gobject_class = G_OBJECT_CLASS (klass);

    gobject_class->finalize = expidus_pointers_helper_finalize;
}



static void
expidus_pointers_helper_init (ExpidusPointersHelper *helper)
{
    ...
    {
        essettings_dbg (XFSD_DEBUG_POINTERS, "initialized xi %d.%d",
                        version->major_version, version->minor_version);

        /* open the channel */
        helper->channel = esconf_channel_get ("pointers");

        /* restore the pointer devices */
        expidus_pointers_helper_restore_devices (helper, NULL);

        /* monitor the channel */
        g_signal_connect (G_OBJECT (helper->channel), "property-changed",
             G_CALLBACK (expidus_pointers_helper_channel_property_changed), helper);

        /* launch syndaemon if required */
        expidus_pointers_helper_syndaemon_check (helper);
        ...
        if (G_LIKELY (xdisplay != NULL))
        {
            ...
            if (gdk_x11_display_error_trap_pop (gdk_display_get_default ()) == 0)
                gdk_window_add_filter (NULL, expidus_pointers_helper_event_filter, helper);
            else
...
static void
expidus_pointers_helper_finalize (GObject *object)
{
    expidus_pointers_helper_syndaemon_stop (EXPIDUS_POINTERS_HELPER (object));

    (*G_OBJECT_CLASS (expidus_pointers_helper_parent_class)->finalize) (object);
}
...
#ifdef HAVE_LIBINPUT
static gboolean
expidus_pointers_is_enabled (Display *xdisplay,
                          XDevice *device)
{
    Atom     prop, type;
...
static gboolean
expidus_pointers_is_libinput (Display *xdisplay,
                           XDevice *device)
{
    Atom     prop, type;
    gulong   n_items, bytes_after;
...
static void
expidus_pointers_helper_syndaemon_stop (ExpidusPointersHelper *helper)
{
#ifdef DEVICE_PROPERTIES
    if (helper->syndaemon_pid != 0)
    {
        essettings_dbg (XFSD_DEBUG_POINTERS, "Killed syndaemon with pid %d",
...
static void
expidus_pointers_helper_syndaemon_check (ExpidusPointersHelper *helper)
{
#ifdef DEVICE_PROPERTIES
    ...
    if (!esconf_channel_get_bool (helper->channel, "/DisableTouchpadWhileTyping", FALSE))
    ...
    expidus_pointers_helper_syndaemon_stop (helper);

    if (have_synaptics)
    {
        disable_duration = esconf_channel_get_double (helper->channel,
        ...
        essettings_dbg (XFSD_DEBUG_POINTERS, "Started syndaemon with pid %d",
                        helper->syndaemon_pid);
    }
#endif
}
...
static gboolean
expidus_pointers_helper_change_button_mapping_swap (guchar   *buttonmap,
                                                 gshort    num_buttons,
                                                 gint      id_1,
                                                 gint      id_2,
                                                 gboolean  reverse)
...
static void
expidus_pointers_helper_change_button_mapping (XDeviceInfo *device_info,
                                            XDevice     *device,
                                            Display     *xdisplay,
                                            gint         right_handed,
                                            gint         reverse_scrolling)
{
...
#ifdef HAVE_LIBINPUT
    if (expidus_pointers_is_libinput (xdisplay, device))
    {
        if (right_handed != -1)
        {
            ...
            expidus_pointers_helper_change_property (device_info, device, xdisplay,
    ...
    if (right_handed != -1)
    {
        ...
        if (expidus_pointers_helper_change_button_mapping_swap (buttonmap, num_buttons,
                                                             1 /* left button */,
                                                             right_button,
                                                             right_handed))
            map_changed = TRUE;
    ...
    if (map_changed)
    {
        ...
        essettings_dbg (XFSD_DEBUG_POINTERS, "[%s] new buttonmap is [%s]",
                        device_info->name, readable_map->str);
        g_string_free (readable_map, TRUE);
    }
    else
    {
        essettings_dbg (XFSD_DEBUG_POINTERS, "[%s] buttonmap not changed",
                        device_info->name);
    }
...
static gint
expidus_pointers_helper_gcd (gint num,
                          gint denom)
{
    /* calc the greatest common divisor using euclidean's algorithm */
    return (denom != 0 ? expidus_pointers_helper_gcd (denom, num % denom) : num);
}



static void
expidus_pointers_helper_change_feedback (XDeviceInfo *device_info,
                                      XDevice     *device,
                                      Display     *xdisplay,
                                      gint         threshold,
    ...
    for (pt = states, n = 0; n < num_feedbacks; n++)
    {
        ...
        if (acceleration >= 0 || acceleration == -1)
        {
            if (acceleration >= 0)
            {
                ...
                gcd = expidus_pointers_helper_gcd (num, denom);
                num /= gcd;
                denom /= gcd;
        ...
        essettings_dbg (XFSD_DEBUG_POINTERS,
...
static void
expidus_pointers_helper_change_mode (XDeviceInfo  *device_info,
                                  XDevice      *device,
                                  Display      *xdisplay,
...
static gchar *
expidus_pointers_helper_device_esconf_name (const gchar *name)
{
    GString     *string;
    const gchar *p;
...
#if defined(DEVICE_PROPERTIES) || defined(HAVE_LIBINPUT)
static void
expidus_pointers_helper_change_property (XDeviceInfo  *device_info,
                                      XDevice      *device,
                                      Display      *xdisplay,
                                      const gchar  *prop_name,
                                      const GValue *value)
{
    ...
    if (prop != XInternAtom (xdisplay, DEVICE_ENABLED, True) &&
        !expidus_pointers_is_enabled (xdisplay, device))
        return;
#endif /* HAVE_LIBINPUT */
    ...
    for (n = 0; n < n_props; n++)
    {
        ...
        if (!gdk_x11_display_error_trap_pop (gdk_display_get_default ()) && rc == Success)
        {
            ...
            else if (G_VALUE_TYPE (value) == G_TYPE_PTR_ARRAY)
            {
                ...
                {
                    g_critical ("Nr device property items (%ld) and esconf value (%d) differ",
                                n_items, array->len);
                    break;
                }
            }
            ...
            for (i = 0; i < n_items; i++)
            {
                ...
                else if (G_VALUE_HOLDS_DOUBLE (val) /* esconf doesn't support floats */
                         && type == float_atom
                         && format == 32)
                {
...
#ifdef DEVICE_PROPERTIES
static void
expidus_pointers_helper_change_properties (gpointer key,
                                        gpointer value,
                                        gpointer user_data)
{
    ExpidusPointerData *pointer_data = user_data;
    const gchar     *prop_name = ((gchar *) key) + pointer_data->prop_name_len;

    expidus_pointers_helper_change_property (pointer_data->device_info,
                                          pointer_data->device,
...
static void
expidus_pointers_helper_restore_devices (ExpidusPointersHelper *helper,
                                      XID                *xid)
{
    Display         *xdisplay = GDK_DISPLAY_XDISPLAY(gdk_display_get_default());
...
#ifdef DEVICE_PROPERTIES
    GHashTable      *props;
    ExpidusPointerData  pointer_data;
#endif
    ...
    for (n = 0; n < ndevices; n++)
    {
        ...
        /* create a valid esconf property name for the device */
        device_name = expidus_pointers_helper_device_esconf_name (device_info->name);

        /* read buttonmap properties */
        g_snprintf (prop, sizeof (prop), "/%s/RightHanded", device_name);
        right_handed = esconf_channel_get_bool (helper->channel, prop, -1);

        g_snprintf (prop, sizeof (prop), "/%s/ReverseScrolling", device_name);
        reverse_scrolling = esconf_channel_get_bool (helper->channel, prop, -1);

        if (right_handed != -1 || reverse_scrolling != -1)
        {
            expidus_pointers_helper_change_button_mapping (device_info, device, xdisplay,
                                                        right_handed, reverse_scrolling);
        }

        /* read feedback settings */
        g_snprintf (prop, sizeof (prop), "/%s/Threshold", device_name);
        threshold = esconf_channel_get_int (helper->channel, prop, -1);

        g_snprintf (prop, sizeof (prop), "/%s/Acceleration", device_name);
        acceleration = esconf_channel_get_double (helper->channel, prop, -1.00);

        if (threshold != -1 || acceleration != -1.00)
        {
            expidus_pointers_helper_change_feedback (device_info, device, xdisplay,
                                                  threshold, acceleration);
        }

        /* read mode settings */
        g_snprintf (prop, sizeof (prop), "/%s/Mode", device_name);
        mode =  esconf_channel_get_string  (helper->channel, prop, NULL);

        if (mode != NULL)
        {
            expidus_pointers_helper_change_mode (device_info, device, xdisplay, mode);
            g_free (mode);
            mode = NULL;
        }
        ...
        props = esconf_channel_get_properties (helper->channel, prop);
        ...
        if (props != NULL)
        {
            ...
            g_hash_table_foreach (props, expidus_pointers_helper_change_properties, &pointer_data);
...
static void
expidus_pointers_helper_channel_property_changed (EsconfChannel      *channel,
                                               const gchar        *property_name,
                                               const GValue       *value,
                                               ExpidusPointersHelper *helper)
{
    ...
    if (names != NULL && g_strv_length (names) >= 2)
    {
        ...
        for (n = 0; n < ndevices; n++)
        {
            ...
            if (strcmp (names[0], device_name) == 0)
            {
                ...
                {
                    expidus_pointers_helper_change_mode (device_info, device, xdisplay,
...
expidus_pointers_helper_event_filter (GdkXEvent *xevent,
                                   GdkEvent  *gdk_event,
                                   gpointer   user_data)
{
    XEvent                     *event = xevent;
    XDevicePresenceNotifyEvent *dpn_event = xevent;
    ExpidusPointersHelper         *helper = EXPIDUS_POINTERS_HELPER (user_data);

    if (event->type == helper->device_presence_event_type)
    {
        /* restore device settings */
        if (dpn_event->devchange == DeviceAdded)
            expidus_pointers_helper_restore_devices (helper, &dpn_event->deviceid);
