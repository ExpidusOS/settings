...
static gchar *expidus_randr_friendly_name (ExpidusRandr *randr,
                                        guint      output,
                                        guint      output_rr_id);
...
static Rotation
expidus_randr_get_safe_rotations (ExpidusRandr *randr,
                               Display   *xdisplay,
                               guint      num_output)
{
    ...
    rot = EXPIDUS_RANDR_ROTATIONS_MASK | EXPIDUS_RANDR_REFLECTIONS_MASK;
...
static ExpidusRRMode *
expidus_randr_list_supported_modes (XRRScreenResources *resources,
                                XRROutputInfo       *output_info)
{
    ExpidusRRMode *modes;
    gint m, n;
    ...
    modes = g_new0 (ExpidusRRMode, output_info->nmode);
...
static void
expidus_randr_guess_relations (ExpidusRandr *randr)
{
    guint n, m;
...
static void
expidus_randr_populate (ExpidusRandr *randr,
                     Display   *xdisplay,
                     GdkWindow *root_window)
{
    ...
    EsconfChannel *display_channel = esconf_channel_new ("displays");
    ...
    randr->priv->modes = g_new0 (ExpidusRRMode *, randr->noutput);
    randr->priv->edid = g_new0 (gchar *, randr->noutput);
    randr->position = g_new0 (ExpidusOutputPosition, randr->noutput);
    ...
    randr->status = g_new0 (ExpidusOutputStatus, randr->noutput);
    ...
    for (m = 0; m < randr->noutput; ++m)
    {
        ...
        randr->priv->modes[m] = expidus_randr_list_supported_modes (randr->priv->resources, randr->priv->output_info[m]);
        ...
        if (randr->priv->has_1_3 && XRRGetOutputPrimary (xdisplay, GDK_WINDOW_XID (root_window)) == randr->priv->resources->outputs[output_ids[m]])
            randr->status[m] = EXPIDUS_OUTPUT_STATUS_PRIMARY;
        else
#endif
            randr->status[m] = EXPIDUS_OUTPUT_STATUS_SECONDARY;
        ...
        else
        {
            ...
            randr->rotations[m] = expidus_randr_get_safe_rotations (randr, xdisplay, m);
        }

        /* fill in the name used by the UI */
        randr->friendly_name[m] = expidus_randr_friendly_name (randr, m, output_ids[m]);

        /* Update display info, primary display may have changed. */
        expidus_randr_save_output (randr, "Default", display_channel, m);

        /* Replace spaces with underscore in name for esconf compatibility */
    ...
    expidus_randr_guess_relations (randr);
...
ExpidusRandr *
expidus_randr_new (GdkDisplay  *display,
                GError     **error)
{
    ExpidusRandr *randr;
    Display   *xdisplay;
    ...
    randr = g_slice_new0 (ExpidusRandr);
    randr->priv = g_slice_new0 (ExpidusRandrPrivate);
    ...
    expidus_randr_populate (randr, xdisplay, root_window);
...
static void
expidus_randr_cleanup (ExpidusRandr *randr)
{
    guint n;
...
void
expidus_randr_free (ExpidusRandr *randr)
{
    expidus_randr_cleanup (randr);

    /* free the structure */
    g_slice_free (ExpidusRandrPrivate, randr->priv);
    g_slice_free (ExpidusRandr, randr);
}



void
expidus_randr_reload (ExpidusRandr *randr)
{
    Display   *xdisplay;
    GdkWindow *root_window;
    ...
    /* get the screen resource */
#ifdef HAS_RANDR_ONE_POINT_THREE
    /* expidus_randr_reload() is only called after a xrandr notification, which
...
void
expidus_randr_save_output (ExpidusRandr     *randr,
                        const gchar   *scheme,
                        ...
                        guint          output)
{
    ...
    esconf_channel_set_string (channel, property, randr->friendly_name[output]);

    /* find the resolution and refresh rate */
    mode = expidus_randr_find_mode_by_id (randr, output, randr->mode[output]);

    /* if no resolution was found, mark it as inactive and stop */
    g_snprintf (property, sizeof (property), "/%s/%s/Active", scheme,
                randr->priv->output_info[output]->name);
    esconf_channel_set_bool (channel, property, mode != NULL);

    g_snprintf (property, sizeof (property), "/%s/%s/EDID", scheme,
                randr->priv->output_info[output]->name);
    esconf_channel_set_string (channel, property, randr->priv->edid[output]);
    ...
    g_snprintf (property, sizeof (property), "/%s/%s/Resolution", scheme,
                randr->priv->output_info[output]->name);
    esconf_channel_set_string (channel, property, str_value);
    g_free (str_value);

    /* save the refresh rate */
    g_snprintf (property, sizeof (property), "/%s/%s/RefreshRate", scheme,
                randr->priv->output_info[output]->name);
    esconf_channel_set_double (channel, property, mode->rate);

    /* convert the rotation into degrees */
    switch (randr->rotation[output] & EXPIDUS_RANDR_ROTATIONS_MASK)
    {
    ...
    g_snprintf (property, sizeof (property), "/%s/%s/Rotation", scheme,
                randr->priv->output_info[output]->name);
    esconf_channel_set_int (channel, property, degrees);

    /* convert the reflection into a string */
    switch (randr->rotation[output] & EXPIDUS_RANDR_REFLECTIONS_MASK)
    {
    ...
    g_snprintf (property, sizeof (property), "/%s/%s/Primary", scheme,
                randr->priv->output_info[output]->name);
    esconf_channel_set_bool (channel, property,
                             randr->status[output] == EXPIDUS_OUTPUT_STATUS_PRIMARY);

    /* save the scale */
    g_snprintf (property, sizeof (property), "/%s/%s/Scale/X", scheme,
                randr->priv->output_info[output]->name);
    esconf_channel_set_double (channel, property, randr->scalex[output]);
    g_snprintf (property, sizeof (property), "/%s/%s/Scale/Y", scheme,
                randr->priv->output_info[output]->name);
    esconf_channel_set_double (channel, property, randr->scaley[output]);
#endif

    /* save the position */
    g_snprintf (property, sizeof (property), "/%s/%s/Position/X", scheme,
                randr->priv->output_info[output]->name);
    esconf_channel_set_int (channel, property, MAX (randr->position[output].x, 0));
    g_snprintf (property, sizeof (property), "/%s/%s/Position/Y", scheme,
                randr->priv->output_info[output]->name);
    esconf_channel_set_int (channel, property, MAX (randr->position[output].y, 0));
}



void
expidus_randr_apply (ExpidusRandr     *randr,
                  const gchar   *scheme,
                  EsconfChannel *channel)
{
    g_return_if_fail (randr != NULL && scheme != NULL);
    ...
    esconf_channel_set_string (channel, "/Schemes/Apply", scheme);
}



void
expidus_randr_load (ExpidusRandr     *randr,
                 const gchar   *scheme,
                 EsconfChannel *channel)
{
...
guint8 *
expidus_randr_read_edid_data (Display  *xdisplay,
                           RROutput  output)
{
    unsigned char *prop;
    int            actual_format;
...
static gchar *
expidus_randr_friendly_name (ExpidusRandr *randr,
                          guint      output,
                          guint      output_rr_id)
{
    Display        *xdisplay;
    ...
    /* get the vendor & size */
    xdisplay = gdk_x11_display_get_xdisplay (randr->priv->display);
    edid_data = expidus_randr_read_edid_data (xdisplay, randr->priv->resources->outputs[output_rr_id]);
...
const ExpidusRRMode *
expidus_randr_find_mode_by_id (ExpidusRandr *randr,
                            guint      output,
                            RRMode     id)
{
    gint n;
...
RRMode
expidus_randr_preferred_mode (ExpidusRandr *randr,
                           guint      output)
{
    RRMode best_mode;
...
RRMode
expidus_randr_clonable_mode (ExpidusRandr *randr)
{
...
gchar *
expidus_randr_get_edid (ExpidusRandr *randr,
                     guint noutput)
{
...
gchar *
expidus_randr_get_output_info_name (ExpidusRandr *randr,
                                 guint noutput)
{
...
const ExpidusRRMode *
expidus_randr_get_modes (ExpidusRandr *randr,
                      guint      output,
                      gint      *nmode)
{
...
gboolean
expidus_randr_get_positions (ExpidusRandr *randr,
                          guint      output,
                          gint      *x,
                          gint      *y)
{
...
guint
expidus_randr_mode_width (const ExpidusRRMode *mode,
                       Rotation          rot)
{
...
guint
expidus_randr_mode_height (const ExpidusRRMode *mode,
                        Rotation          rot)
{
    g_return_val_if_fail (mode != NULL, 0);
