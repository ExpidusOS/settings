...
#include "expidus-settings-editor-box.h"
#include "expidus-settings-prop-dialog.h"
#include "expidus-settings-cell-renderer.h"



struct _ExpidusSettingsEditorBoxClass
{
    GtkBoxClass __parent__;
};

struct _ExpidusSettingsEditorBox
{
    GtkBox __parent__;
...
static void     expidus_settings_editor_box_finalize             (GObject                *object);

static void     expidus_settings_editor_box_load_channels        (ExpidusSettingsEditorBox  *self);
static void     expidus_settings_editor_box_channel_changed      (GtkTreeSelection       *selection,
                                                               ExpidusSettingsEditorBox  *self);
static gboolean expidus_settings_editor_box_channel_menu         (ExpidusSettingsEditorBox  *self);
static gboolean expidus_settings_editor_box_channel_button_press (GtkWidget              *treeview,
                                                               GdkEventButton         *event,
                                                               ExpidusSettingsEditorBox  *self);
static void     expidus_settings_editor_box_value_changed        (GtkCellRenderer        *renderer,
                                                               const gchar            *path,
                                                               const GValue           *new_value,
                                                               ExpidusSettingsEditorBox  *self);
static void     expidus_settings_editor_box_selection_changed    (GtkTreeSelection       *selection,
                                                               ExpidusSettingsEditorBox  *self);
static gboolean expidus_settings_editor_box_query_tooltip        (GtkWidget              *treeview,
                                                               gint                    x,
                                                               gint                    y,
                                                               gboolean                keyboard_mode,
                                                               GtkTooltip             *tooltip,
                                                               ExpidusSettingsEditorBox  *self);
static void     expidus_settings_editor_box_row_activated        (GtkTreeView            *treeview,
                                                               GtkTreePath            *path,
                                                               GtkTreeViewColumn      *column,
                                                               ExpidusSettingsEditorBox  *self);
static gboolean expidus_settings_editor_box_key_press_event      (GtkTreeView            *treeview,
                                                               GdkEventKey            *event,
                                                               ExpidusSettingsEditorBox  *self);
static void     expidus_settings_editor_box_property_new         (ExpidusSettingsEditorBox  *self);
static void     expidus_settings_editor_box_property_edit        (ExpidusSettingsEditorBox  *self);
static void     expidus_settings_editor_box_property_reset       (ExpidusSettingsEditorBox  *self);
...
G_DEFINE_TYPE (ExpidusSettingsEditorBox, expidus_settings_editor_box, GTK_TYPE_BOX)

static void expidus_settings_editor_box_get_property (GObject    *object,
                                                   guint       prop_id,
                                                   GValue     *value,
                                                   GParamSpec *pspec)
{
    ExpidusSettingsEditorBox *self;
    self = EXPIDUS_SETTINGS_EDITOR_BOX (object);
...
static void expidus_settings_editor_box_set_property (GObject      *object,
                                                   guint         prop_id,
                                                   const GValue *value,
                                                   GParamSpec   *pspec)
{
...
static void
expidus_settings_editor_box_init (ExpidusSettingsEditorBox *self)
{
    GtkWidget         *paned;
    GtkWidget         *scroll;
    GtkWidget         *treeview;
    ...
    self->channels_store = gtk_list_store_new (N_CHANNEL_COLUMNS,
                                                 G_TYPE_STRING);
    expidus_settings_editor_box_load_channels (self);
    ...
    g_signal_connect_swapped (G_OBJECT (treeview), "popup-menu",
        G_CALLBACK (expidus_settings_editor_box_channel_menu), self);
    g_signal_connect (G_OBJECT (treeview), "button-press-event",
        G_CALLBACK (expidus_settings_editor_box_channel_button_press), self);

    selection = gtk_tree_view_get_selection (GTK_TREE_VIEW (treeview));
    gtk_tree_selection_set_mode (selection, GTK_SELECTION_BROWSE);
    g_signal_connect (G_OBJECT (selection), "changed",
        G_CALLBACK (expidus_settings_editor_box_channel_changed), self);
    ...
    g_signal_connect (G_OBJECT (treeview), "query-tooltip",
        G_CALLBACK (expidus_settings_editor_box_query_tooltip), self);
    g_signal_connect (G_OBJECT (treeview), "row-activated",
        G_CALLBACK (expidus_settings_editor_box_row_activated), self);
    g_signal_connect (G_OBJECT (treeview), "key-press-event",
        G_CALLBACK (expidus_settings_editor_box_key_press_event), self);

    selection = gtk_tree_view_get_selection (GTK_TREE_VIEW (treeview));
    g_signal_connect (G_OBJECT (selection), "changed",
        G_CALLBACK (expidus_settings_editor_box_selection_changed), self);
    ...
    render = expidus_settings_cell_renderer_new ();
    ...
    g_signal_connect (G_OBJECT (render), "value-changed",
        G_CALLBACK (expidus_settings_editor_box_value_changed), self);
    ...
    g_signal_connect_swapped (G_OBJECT (button), "clicked",
        G_CALLBACK (expidus_settings_editor_box_property_new), self);
    ...
    gtk_widget_show (button);
    g_signal_connect_swapped (G_OBJECT (button), "clicked",
        G_CALLBACK (expidus_settings_editor_box_property_edit), self);
    ...
    gtk_widget_show (button);
    g_signal_connect_swapped (G_OBJECT (button), "clicked",
        G_CALLBACK (expidus_settings_editor_box_property_reset), self);
}

static void
expidus_settings_editor_box_class_init (ExpidusSettingsEditorBoxClass *klass)
{
    GObjectClass   *gobject_class;

    gobject_class = G_OBJECT_CLASS (klass);

    gobject_class->set_property = expidus_settings_editor_box_set_property;
    gobject_class->get_property = expidus_settings_editor_box_get_property;
    ...
    gobject_class->finalize = expidus_settings_editor_box_finalize;
}


static void
expidus_settings_editor_box_finalize (GObject *object)
{
    GSList *li, *lnext;

    ExpidusSettingsEditorBox *self = EXPIDUS_SETTINGS_EDITOR_BOX (object);
    ...
    G_OBJECT_CLASS (expidus_settings_editor_box_parent_class)->finalize (object);
}

static void
expidus_settings_editor_box_load_channels (ExpidusSettingsEditorBox *self)
{
    gchar       **channel_names;
    guint         i;
    ...
    channel_names = esconf_list_channels ();
...
static const gchar *
expidus_settings_editor_box_type_name (const GValue *value)
{
    if (G_UNLIKELY (value == NULL))
        return _("Empty");

    if (G_UNLIKELY (G_VALUE_TYPE (value) == expidus_settings_array_type ()))
        return _("Array");
...
static void
expidus_settings_editor_box_property_load (const gchar               *property,
                                        const GValue              *value,
                                        ExpidusSettingsEditorBox     *self,
        ...
        if (!found_parent)
        {
            ...
            else
            {
                ...
                gtk_tree_store_set (GTK_TREE_STORE (model), &child_iter,
                                    ...
                                    PROP_COLUMN_TYPE_NAME, expidus_settings_editor_box_type_name (value),
                                    PROP_COLUMN_LOCKED, esconf_channel_is_property_locked (self->props_channel, property),
...
static gboolean
expidus_settings_editor_box_property_find (GtkTreeModel *model,
                                        GtkTreePath  *path,
                                        GtkTreeIter  *iter,
                                        gpointer      data)
{
...
static void
expidus_settings_editor_box_property_changed (EsconfChannel            *channel,
                                           ...
                                           ExpidusSettingsEditorBox    *self)
{
    ...
    {
        expidus_settings_editor_box_property_load (property, value, self, &path);
    ...
    else
    {
        ...
        gtk_tree_model_foreach (model, expidus_settings_editor_box_property_find, context);
            ...
            if (gtk_tree_model_get_iter (model, &child_iter, context->path))
            {
                ...
                else
                {
                    ...
                    while (has_parent)
                    {
                        ...
                        /* nope it points to a real esconf property */
                        if (!empty_prop)
                            break;
    ...
    expidus_settings_editor_box_selection_changed (selection, self);
}



static void
expidus_settings_editor_box_property_load_hash (gpointer key,
                                             gpointer value,
                                             gpointer data)
{
    expidus_settings_editor_box_property_load (key, value, data, NULL);
}



static void
expidus_settings_editor_box_properties_load (ExpidusSettingsEditorBox *self,
                                          EsconfChannel         *channel)
{
    ...
    if (self->props_channel != NULL)
    {
        g_signal_handlers_block_by_func (G_OBJECT (self->props_channel),
            G_CALLBACK (expidus_settings_editor_box_property_changed), self);
        g_object_unref (G_OBJECT (self->props_channel));
    ...
    self->props_channel = (EsconfChannel *) g_object_ref (G_OBJECT (channel));

    props = esconf_channel_get_properties (channel, NULL);
    if (G_LIKELY (props != NULL))
    {
        g_hash_table_foreach (props, expidus_settings_editor_box_property_load_hash, self);
...
static void
expidus_settings_editor_box_channel_changed (GtkTreeSelection         *selection,
                                          ExpidusSettingsEditorBox *self)
{
    GtkTreeIter    iter;
    GValue         value = { 0, };
    ...
    if (gtk_tree_selection_get_selected (selection, NULL, &iter))
    {
        ...
        channel = esconf_channel_new (g_value_get_string (&value));

        locked = esconf_channel_is_property_locked (channel, "/");
        gtk_widget_set_sensitive (self->button_new, !locked);

        expidus_settings_editor_box_properties_load (self, channel);
...
static void
expidus_settings_editor_box_channel_reset (ExpidusSettingsEditorBox *self)
{
    gchar             *channel_name;
    GtkTreeSelection  *selection;
    GtkTreeIter        iter;
    ...
    if (expidus_dialog_confirm (GTK_WINDOW (gtk_widget_get_toplevel(GTK_WIDGET(self))),
                             "document-revert",
                             _("_Reset Channel"),
                             ...
                             channel_name))
    {
        ...
        esconf_channel_reset_property (self->props_channel, "/", TRUE);
...
static void
expidus_settings_editor_box_channel_monitor_changed (EsconfChannel *channel,
                                                  const gchar   *property,
                                                  const GValue  *value,
                                                  GtkWidget     *window)
{
...
static void
expidus_settings_editor_box_channel_monitor_response (GtkWidget     *window,
                                                   gint           response_id,
                                                   EsconfChannel *channel)
{
    GtkTextBuffer *buffer;
    ...
    else
    {
        g_signal_handlers_disconnect_by_func (G_OBJECT (channel),
            G_CALLBACK (expidus_settings_editor_box_channel_monitor_changed), window);
...
static void
expidus_settings_editor_box_channel_monitor (ExpidusSettingsEditorBox *self)
{
    GtkWidget     *window;
    gchar         *channel_name;
    gchar         *title;
    ...
    window = expidus_titled_dialog_new ();
    ...
    g_signal_connect (G_OBJECT (window), "response",
        G_CALLBACK (expidus_settings_editor_box_channel_monitor_response),
...
static gboolean
expidus_settings_editor_box_channel_menu (ExpidusSettingsEditorBox *self)
{
    GtkWidget *menu;
    GtkWidget *mi;
    gchar     *channel_name;
    ...
    g_signal_connect_swapped (G_OBJECT (mi), "activate",
        G_CALLBACK (expidus_settings_editor_box_channel_reset), self);
    gtk_widget_show (mi);

    mi = gtk_menu_item_new_with_mnemonic (_("_Monitor"));
    gtk_menu_shell_append (GTK_MENU_SHELL (menu), mi);
    g_signal_connect_swapped (G_OBJECT (mi), "activate",
        G_CALLBACK (expidus_settings_editor_box_channel_monitor), self);
    gtk_widget_show (mi);
    ...
    gtk_menu_shell_append (GTK_MENU_SHELL (menu), mi);
    g_signal_connect_swapped (G_OBJECT (mi), "activate",
        G_CALLBACK (expidus_settings_editor_box_load_channels), self);
    gtk_widget_show (mi);
...
static gboolean
expidus_settings_editor_box_channel_button_press (GtkWidget                *treeview,
                                               GdkEventButton           *event,
                                               ExpidusSettingsEditorBox    *self)
{
    guint        modifiers;
    ...
    if (event->type == GDK_BUTTON_PRESS)
    {
        ...
        if (event->button == 3 || (event->button == 1 && modifiers == GDK_CONTROL_MASK))
        {
                                                ...
                                                &path, NULL, NULL, NULL))
             {
                 ...
                 return expidus_settings_editor_box_channel_menu (self);;
             }
        }
    }
...
static gchar *
expidus_settings_editor_box_selected (ExpidusSettingsEditorBox    *self,
                                   ...
                                   gboolean                 *is_array)
{
    ...
    if (gtk_tree_selection_get_selected (selection, NULL, &iter))
    {
        ...
        else if (is_array != NULL)
        {
            ...
            *is_array = g_strcmp0 (type_name, g_type_name (expidus_settings_array_type ())) == 0;
            g_free (type_name);
        }
    }
...
static void
expidus_settings_editor_box_value_changed (GtkCellRenderer          *renderer,
                                        ...
                                        ExpidusSettingsEditorBox    *self)
{
    ...
    g_return_if_fail (EXPIDUS_IS_SETTINGS_EDITOR_BOX (self));
        ...
        && gtk_tree_model_get_iter (model, &iter, path))
    {
        ...
        {
            if (!esconf_channel_is_property_locked (self->props_channel, property))
                esconf_channel_set_property (self->props_channel, property, new_value);
            g_free (property);
        }
    }
    gtk_tree_path_free (path);
...
static void
expidus_settings_editor_box_selection_changed (GtkTreeSelection         *selection,
                                            ExpidusSettingsEditorBox    *self)
{
    gchar    *property;
    ...
    g_return_if_fail (self->props_channel == NULL
                      || ESCONF_IS_CHANNEL (self->props_channel));

    /* do nothing if the entre channel is locked */
    if (self->props_channel != NULL
        && gtk_widget_get_sensitive (self->button_new))
    {
        property = expidus_settings_editor_box_selected (self, &is_real_prop, &is_array);

        can_edit = !esconf_channel_is_property_locked (self->props_channel, property);
...
static gboolean
expidus_settings_editor_box_query_tooltip (GtkWidget                *treeview,
                                        gint                      x,
                                        gint                      y,
                                        gboolean                  keyboard_mode,
                                        GtkTooltip               *tooltip,
...
static void
expidus_settings_editor_box_row_activated (GtkTreeView              *treeview,
                                        GtkTreePath              *path,
                                        GtkTreeViewColumn        *column,
                                        ExpidusSettingsEditorBox    *self)
{
    ...
    if (gtk_tree_model_get_iter (model, &iter, path))
    {
        ...
        {
            expidus_settings_editor_box_property_edit (self);
        }
    }
}
...
static gboolean
expidus_settings_editor_box_key_press_event (GtkTreeView              *treeview,
                                          GdkEventKey              *event,
                                          ExpidusSettingsEditorBox    *self)
{
    ...
    {
        expidus_settings_editor_box_property_reset (self);
        return TRUE;
    }
    else if (event->keyval == GDK_KEY_Insert
             && gtk_widget_get_sensitive (self->button_new))
    {
        expidus_settings_editor_box_property_new (self);
        return TRUE;
    }
...
static void
expidus_settings_editor_box_property_dialog (ExpidusSettingsEditorBox *self,
                                          gboolean               make_new)
{
    GtkWidget *prop_dialog;
    gchar     *property;

    property = expidus_settings_editor_box_selected (self, NULL, NULL);

    prop_dialog = expidus_settings_prop_dialog_new (GTK_WINDOW(gtk_widget_get_toplevel(GTK_WIDGET(self))),
                                                 self->props_channel,
                                                 make_new ? NULL : property);
    if (make_new)
    {
        /* hint for the parent property based on selected property */
        expidus_settings_prop_dialog_set_parent_property (
            EXPIDUS_SETTINGS_PROP_DIALOG (prop_dialog), property);
    }
...
static void
expidus_settings_editor_box_property_new (ExpidusSettingsEditorBox *self)
{
    expidus_settings_editor_box_property_dialog (self, TRUE);
}



static void
expidus_settings_editor_box_property_edit (ExpidusSettingsEditorBox *self)
{
    expidus_settings_editor_box_property_dialog (self, FALSE);
}



static void
expidus_settings_editor_box_property_reset (ExpidusSettingsEditorBox *self)
{
    gchar *property;

    property = expidus_settings_editor_box_selected (self, NULL, NULL);
    if (property != NULL
        && expidus_dialog_confirm (GTK_WINDOW(gtk_widget_get_toplevel(GTK_WIDGET(self))),
    ...
    {
        esconf_channel_reset_property (self->props_channel, property, FALSE);
    }
...
GtkWidget *
expidus_settings_editor_box_new (gint paned_pos)
{
    return g_object_new (EXPIDUS_TYPE_SETTINGS_EDITOR_BOX,
                         "paned-pos", paned_pos,
                         NULL);
}
