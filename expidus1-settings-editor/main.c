...
static void
save_window_size (GtkWidget *dialog, ExpidusSettingsEditorBox *settings_editor)
{
	...
	if ((state & (GDK_WINDOW_STATE_MAXIMIZED | GDK_WINDOW_STATE_FULLSCREEN)) == 0)
	{
		...
		esconf_channel_set_int (channel, "/last/window-width", width),
			esconf_channel_set_int (channel, "/last/window-height", height);
		esconf_channel_set_int (channel, "/last/paned-position", paned_pos);
...
settings_dialog_response (GtkWidget *dialog,
						  gint       response_id,
						  ExpidusSettingsEditorBox *settings_editor)
{
    if (response_id == GTK_RESPONSE_HELP)
		expidus_dialog_show_help_with_version (GTK_WINDOW (dialog),
                                            "expidus1-settings",
                                            "editor", NULL,
                                            EXPIDUS1_SETTINGS_VERSION_SHORT);
	else
	{
...
main(gint argc, gchar **argv)
{
	...
	channel = esconf_channel_new ("expidus1-settings-editor");

	settings_editor = expidus_settings_editor_box_new (
		esconf_channel_get_int (channel, "/last/paned-position", 180));

	if (G_UNLIKELY (opt_socket_id == 0))
    {
		dialog = expidus_titled_dialog_new_with_mixed_buttons (_("Settings Editor"), NULL,
					GTK_DIALOG_DESTROY_WITH_PARENT,
					"help-browser", _("_Help"), GTK_RESPONSE_HELP,
					"window-close-symbolic", _("_Close"), GTK_RESPONSE_OK,
					NULL);

		gtk_window_set_icon_name (GTK_WINDOW (dialog), "com.expidus.settings.editor");
		gtk_window_set_type_hint (GTK_WINDOW (dialog), GDK_WINDOW_TYPE_HINT_NORMAL);
		gtk_window_set_default_size (GTK_WINDOW (dialog),
          esconf_channel_get_int (channel, "/last/window-width", 640),
          esconf_channel_get_int (channel, "/last/window-height", 500));
