...
struct _ExpidusSettingsPropDialogClass
{
    GtkDialogClass __parent__;
};

struct _ExpidusSettingsPropDialog
{
...
static void     expidus_settings_prop_dialog_finalize             (GObject                   *object);
static void     expidus_settings_prop_dialog_response             (GtkDialog                 *widget,
                                                                gint                       response_id);
static void     expidus_settings_prop_dialog_visible_bind         (GtkWidget                 *widget,
                                                                GtkWidget                 *label);
static void     expidus_settings_prop_dialog_entry_validate       (GtkWidget                 *entry,
                                                                ExpidusSettingsPropDialog    *dialog);
static void     expidus_settings_prop_dialog_button_toggled       (GtkWidget                 *button);
static void     expidus_settings_prop_dialog_type_changed         (GtkWidget                 *combo,
                                                                ExpidusSettingsPropDialog    *dialog);



G_DEFINE_TYPE (ExpidusSettingsPropDialog, expidus_settings_prop_dialog, GTK_TYPE_DIALOG)
...
static void
expidus_settings_prop_dialog_class_init (ExpidusSettingsPropDialogClass *klass)
{
    GObjectClass   *gobject_class;
    GtkDialogClass *gtkdialog_class;

    gobject_class = G_OBJECT_CLASS (klass);
    gobject_class->finalize = expidus_settings_prop_dialog_finalize;

    gtkdialog_class = GTK_DIALOG_CLASS (klass);
    gtkdialog_class->response = expidus_settings_prop_dialog_response;
}



static void
expidus_settings_prop_dialog_init (ExpidusSettingsPropDialog *dialog)
{
    GtkWidget       *table;
    ...
    expidus_settings_prop_dialog_visible_bind (entry, label);
    g_signal_connect (G_OBJECT (entry), "changed",
        G_CALLBACK (expidus_settings_prop_dialog_entry_validate), dialog);
    ...
    expidus_settings_prop_dialog_visible_bind (combo, label);
    gtk_combo_box_set_active (GTK_COMBO_BOX (combo), 0);
    g_signal_connect (G_OBJECT (combo), "changed",
        G_CALLBACK (expidus_settings_prop_dialog_type_changed), dialog);
    ...
    expidus_settings_prop_dialog_visible_bind (spin, label);
    ...
    g_signal_connect (G_OBJECT (toggle), "toggled",
        G_CALLBACK (expidus_settings_prop_dialog_button_toggled), NULL);
    gtk_grid_attach (GTK_GRID (table), toggle, 1, 4, 1, 1);
    expidus_settings_prop_dialog_visible_bind (toggle, label);
}



static void
expidus_settings_prop_dialog_finalize (GObject *object)
{
    ExpidusSettingsPropDialog *dialog = EXPIDUS_SETTINGS_PROP_DIALOG (object);
    ...
    G_OBJECT_CLASS (expidus_settings_prop_dialog_parent_class)->finalize (object);
}



static void
expidus_settings_prop_dialog_response (GtkDialog *widget,
                                    gint       response_id)
{
    ExpidusSettingsPropDialog *dialog = EXPIDUS_SETTINGS_PROP_DIALOG (widget);
    ...
    g_return_if_fail (ESCONF_IS_CHANNEL (dialog->channel));
    ...
    if (response_id == GTK_RESPONSE_OK)
    {
        ...
        if (G_IS_VALUE (&value))
        {
            esconf_channel_set_property (dialog->channel, property, &value);
            g_value_unset (&value);
        }
    }
}
...
static void
expidus_settings_prop_dialog_visible_changed (GtkWidget  *widget,
                                           GParamSpec *pspec,
                                           GtkWidget  *label)
{
...
static void
expidus_settings_prop_dialog_sensitive_changed (GtkWidget  *widget,
                                             GParamSpec *pspec,
                                             GtkWidget  *label)
{
    g_return_if_fail (GTK_IS_WIDGET (widget));
...
static void
expidus_settings_prop_dialog_visible_bind (GtkWidget *widget,
                                        GtkWidget *label)
{
    gtk_label_set_mnemonic_widget (GTK_LABEL (label), widget);
    g_signal_connect (G_OBJECT (widget), "notify::visible",
        G_CALLBACK (expidus_settings_prop_dialog_visible_changed), label);
    g_signal_connect (G_OBJECT (widget), "notify::sensitive",
        G_CALLBACK (expidus_settings_prop_dialog_sensitive_changed), label);
}



/* Copied from esconfd/esconf-backend.c */
static gboolean
...
static void
expidus_settings_prop_dialog_entry_validate (GtkWidget              *entry,
                                          ExpidusSettingsPropDialog *dialog)
{
    GtkWidget   *save_button;
    const gchar *text;
    ...
    if (text != NULL && *text != '\0')
    {
        is_valid = esconf_property_is_valid (text, &error);
...
static void
expidus_settings_prop_dialog_button_toggled (GtkWidget *button)
{
    const gchar *label;
...
static void
expidus_settings_prop_dialog_type_changed (GtkWidget              *combo,
                                        ExpidusSettingsPropDialog *dialog)
{
    gint          active;
    ValueTypes   *value_type;
...
static void
expidus_settings_prop_dialog_type_set_active (ExpidusSettingsPropDialog *dialog,
                                           GType                   value_type)
{
    guint i;
...
GtkWidget *
expidus_settings_prop_dialog_new (GtkWindow     *parent,
                               EsconfChannel *channel,
                               const gchar   *property)
{
    ExpidusSettingsPropDialog *dialog;

    g_return_val_if_fail (ESCONF_IS_CHANNEL (channel), NULL);
    g_return_val_if_fail (parent == NULL || GTK_IS_WINDOW (parent), NULL);

    dialog = g_object_new (EXPIDUS_TYPE_SETTINGS_PROP_DIALOG, NULL);

    dialog->channel = (EsconfChannel *) g_object_ref (G_OBJECT (channel));
    ...
    if (property != NULL)
    {
        ...
        if (esconf_channel_get_property (channel, property, &dialog->prop_value))
        {
            expidus_settings_prop_dialog_type_set_active (dialog,
...
void
expidus_settings_prop_dialog_set_parent_property (ExpidusSettingsPropDialog *dialog,
                                               const gchar            *property)
{
    gchar *p;
    gint   length = -1;
    ...
    g_return_if_fail (EXPIDUS_IS_SETTINGS_PROP_DIALOG (dialog));
