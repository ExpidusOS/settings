...
static void     expidus_mime_chooser_finalize          (GObject           *object);
static void     expidus_mime_chooser_row_activated     (GtkTreeView       *tree_view,
                                                     GtkTreePath       *path,
                                                     GtkTreeViewColumn *column,
                                                     ExpidusMimeChooser   *chooser);
static gboolean expidus_mime_chooser_row_can_select    (GtkTreeSelection  *selection,
                                                     GtkTreeModel      *model,
                                                     GtkTreePath       *path,
                                                     gboolean           path_currently_selected,
                                                     gpointer           data);
static void     expidus_mime_chooser_update_accept     (ExpidusMimeChooser   *chooser);
static void     expidus_mime_chooser_notify_expanded   (GtkExpander       *expander,
                                                     GParamSpec        *pspec,
                                                     ExpidusMimeChooser   *chooser);
static void     expidus_mime_chooser_browse_command    (GtkWidget         *button,
                                                     ExpidusMimeChooser   *chooser);



struct _ExpidusMimeChooserClass
{
    GtkDialogClass __parent__;
};

struct _ExpidusMimeChooser
{
...
G_DEFINE_TYPE (ExpidusMimeChooser, expidus_mime_chooser, GTK_TYPE_DIALOG)



static void
expidus_mime_chooser_class_init (ExpidusMimeChooserClass *klass)
{
    GObjectClass *gobject_class;

    gobject_class = G_OBJECT_CLASS (klass);
    gobject_class->finalize = expidus_mime_chooser_finalize;
}



static void
expidus_mime_chooser_init (ExpidusMimeChooser *chooser)
{
    GtkWidget         *vbox;
    GtkWidget         *scroll;
    GtkWidget         *area;
    ...
    g_signal_connect (G_OBJECT (treeview), "row-activated",
        G_CALLBACK (expidus_mime_chooser_row_activated), chooser);
    gtk_widget_show (treeview);
    chooser->treeview = treeview;

    selection = gtk_tree_view_get_selection (GTK_TREE_VIEW (treeview));
    gtk_tree_selection_set_select_function (selection,
        expidus_mime_chooser_row_can_select, NULL, NULL);
    g_signal_connect_swapped (G_OBJECT (selection), "changed",
        G_CALLBACK (expidus_mime_chooser_update_accept), chooser);
    ...
    g_signal_connect (G_OBJECT (expander), "notify::expanded",
        G_CALLBACK (expidus_mime_chooser_notify_expanded), chooser);
    ...
    g_signal_connect (G_OBJECT (button), "clicked",
         G_CALLBACK (expidus_mime_chooser_browse_command), chooser);
...
static void
expidus_mime_chooser_finalize (GObject *object)
{
    ExpidusMimeChooser *chooser = EXPIDUS_MIME_CHOOSER (object);

    g_object_unref (G_OBJECT (chooser->model));

    (*G_OBJECT_CLASS (expidus_mime_chooser_parent_class)->finalize) (object);
}



static GAppInfo *
expidus_mime_chooser_get_selected (ExpidusMimeChooser *chooser)
{
    GtkTreeIter       iter;
    GAppInfo         *app_info = NULL;
    GtkTreeSelection *selection;

    g_return_val_if_fail (EXPIDUS_IS_MIME_CHOOSER (chooser), NULL);
...
static void
expidus_mime_chooser_row_activated (GtkTreeView       *tree_view,
                                 GtkTreePath       *path,
                                 GtkTreeViewColumn *column,
                                 ExpidusMimeChooser   *chooser)
{
...
static gboolean
expidus_mime_chooser_row_can_select (GtkTreeSelection  *selection,
                                  GtkTreeModel      *model,
                                  GtkTreePath       *path,
                                  gboolean           path_currently_selected,
                                  gpointer           data)
...
static void
expidus_mime_chooser_update_accept (ExpidusMimeChooser *chooser)
{
    ...
    {
        app_info = expidus_mime_chooser_get_selected (chooser);
        if (app_info != NULL)
        {
            can_open = TRUE;
...
static void
expidus_mime_chooser_notify_expanded (GtkExpander     *expander,
                                   GParamSpec      *pspec,
                                   ExpidusMimeChooser *chooser)
{
    gboolean          expanded;
    ...
    expidus_mime_chooser_update_accept (chooser);
}



static void
expidus_mime_chooser_browse_command (GtkWidget       *button,
                                  ExpidusMimeChooser *dialog)
{
    GtkWidget     *chooser;
...
static void
expidus_mime_chooser_model_append (GtkTreeStore *model,
                                const gchar  *title,
...
static gint
expidus_mime_chooser_compare_app_info (gconstpointer a,
                                    gconstpointer b)
{
...
static gint
expidus_mime_chooser_sort_app_info (gconstpointer a,
                                 gconstpointer b)
{
...
void
expidus_mime_chooser_set_mime_type (ExpidusMimeChooser *chooser,
                                 const gchar     *mime_type)
{
    ...
    g_return_if_fail (EXPIDUS_IS_MIME_CHOOSER (chooser));
    ...
    expidus_mime_chooser_model_append (chooser->model,
    ...
    for (li = all; li != NULL; li = li->next)
    {
        if (g_list_find_custom (recommended, li->data,
                                expidus_mime_chooser_compare_app_info) == NULL)
        {
    ...
    other = g_list_sort (other, expidus_mime_chooser_sort_app_info);
    expidus_mime_chooser_model_append (chooser->model,
...
GAppInfo *
expidus_mime_chooser_get_app_info (ExpidusMimeChooser *chooser)
{
    const gchar *exec;
    GAppInfo    *app_info;
    gchar       *path;
    ...
    if (gtk_expander_get_expanded (GTK_EXPANDER (chooser->expander)))
    {
        ...
        {
            expidus_dialog_show_error (GTK_WINDOW (chooser), error, _("Failed to add new application \"%s\""), name);
            g_error_free (error);
        }
    ...
    {
        return expidus_mime_chooser_get_selected (chooser);
    }
}
