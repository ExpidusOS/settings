...
static GtkWidget *
get_helper_error_dialog (ExpidusMimeHelperCategory  category,
                         GError                 *error,
                         gboolean               *return_value)
{
  GtkWidget *dialog;
...
main (int argc, char **argv)
{
  ...
  g_assert (EXPIDUS_MIME_HELPER_N_CATEGORIES == G_N_ELEMENTS (CATEGORY_EXEC_ERRORS));
   ...
   * FileManager and TerminalEmulator), since the expidus-mime-helper utility will
  ...
  gtk_window_set_default_icon_name ("com.expidus.settings.preferred-applications");

  /* check for the action to perform */
  if (opt_launch_type != NULL)
    {
      /* try to parse the type */
      if (!expidus_mime_helper_category_from_string (opt_launch_type, &category))
        {
      ...
      database = expidus_mime_helper_database_get ();
      helper = expidus_mime_helper_database_get_default (database, category);

      /* check if we have a valid helper */
      if (G_UNLIKELY (helper == NULL))
        {
          /* ask the user to choose a default helper for category */
          dialog = expidus_mime_helper_launcher_dialog_new (category);
          if (startup_id != NULL)
            gtk_window_set_startup_id (GTK_WINDOW (dialog), startup_id);
          if (gtk_dialog_run (GTK_DIALOG (dialog)) == GTK_RESPONSE_OK)
            helper = expidus_mime_helper_database_get_default (database, category);
          else
            expidus_mime_helper_database_clear_default (database, category, NULL);
      ...
      if (G_LIKELY (helper != NULL))
        {
          ...
          if (!expidus_mime_helper_execute (helper, NULL, (argc > 1) ? argv[1] : NULL, &error))
            {
              if (!expidus_mime_helper_database_get_dismissed (database, category))
                {
                  ...
                  if (dismissed)
                    {
                      expidus_mime_helper_database_set_dismissed (database, category, dismissed);
                    }
                }
              g_error_free (error);
              result = EXIT_FAILURE;
    ...
    {
      /* try to parse the type */
      if (!expidus_mime_helper_category_from_string (opt_query_type, &category))
        {
      ...
      g_print ("%s\n", expidus_mime_helper_get_id (helper));
    }
  else
    {
      result = EXIT_FAILURE;
