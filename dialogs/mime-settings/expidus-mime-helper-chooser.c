...
static void expidus_mime_helper_chooser_finalize     (GObject                *object);
static void expidus_mime_helper_chooser_get_property (GObject                *object,
                                             guint                   prop_id,
                                             GValue                 *value,
                                             GParamSpec             *pspec);
static void expidus_mime_helper_chooser_set_property (GObject                *object,
                                             guint                   prop_id,
                                             const GValue           *value,
                                             GParamSpec             *pspec);
static void expidus_mime_helper_chooser_update       (ExpidusMimeHelperChooser       *chooser);
static void expidus_mime_helper_chooser_pressed      (ExpidusMimeHelperChooser       *chooser,
                                             GtkWidget              *button);



struct _ExpidusMimeHelperChooserClass
{
  GtkBinClass __parent__;
};

struct _ExpidusMimeHelperChooser
{
  GtkBin __parent__;
  ...
  ExpidusMimeHelperDatabase *database;
...
G_DEFINE_TYPE (ExpidusMimeHelperChooser, expidus_mime_helper_chooser, GTK_TYPE_BIN)



static void
expidus_mime_helper_chooser_class_init (ExpidusMimeHelperChooserClass *klass)
{
  GObjectClass *gobject_class;

  gobject_class = G_OBJECT_CLASS (klass);
  gobject_class->finalize = expidus_mime_helper_chooser_finalize;
  gobject_class->get_property = expidus_mime_helper_chooser_get_property;
  gobject_class->set_property = expidus_mime_helper_chooser_set_property;

  /**
   * ExpidusMimeHelperChooser:category:
   *
   * The #ExpidusMimeHelperCategory which should be configured by this
   * #ExpidusMimeHelperChooser. See expidus_mime_helper_chooser_get_category() and
   * expidus_mime_helper_chooser_set_category() for details.
   **/
  g_object_class_install_property (gobject_class,
                                   ...
                                   g_param_spec_enum ("category",
                                                      "Helper category",
                                                      "Helper category",
                                                      EXPIDUS_MIME_TYPE_MIME_HELPER_CATEGORY,
                                                      EXPIDUS_MIME_HELPER_WEBBROWSER,
                                                      ENDO_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  /**
   * ExpidusMimeHelperChooser:is-valid:
   *
   * %TRUE if a valid #ExpidusMimeHelper is selected by
   * this #ExpidusMimeHelperChooser, else %FALSE.
   **/
  g_object_class_install_property (gobject_class,
                                   PROP_IS_VALID,
...
static void
expidus_mime_helper_chooser_init (ExpidusMimeHelperChooser *chooser)
{
  ...
  chooser->database = expidus_mime_helper_database_get ();

  /*gtk_widget_push_composite_child ();*/

  button = gtk_button_new ();
  g_signal_connect_swapped (G_OBJECT (button), "pressed", G_CALLBACK (expidus_mime_helper_chooser_pressed), chooser);
...
static void
expidus_mime_helper_chooser_finalize (GObject *object)
{
  ExpidusMimeHelperChooser *chooser = EXPIDUS_MIME_HELPER_CHOOSER (object);

  g_object_unref (G_OBJECT (chooser->database));

  (*G_OBJECT_CLASS (expidus_mime_helper_chooser_parent_class)->finalize) (object);
}



static void
expidus_mime_helper_chooser_get_property (GObject    *object,
                                 guint       prop_id,
                                 GValue     *value,
                                 GParamSpec *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_CATEGORY:
      g_value_set_enum (value, expidus_mime_helper_chooser_get_category (chooser));
      break;

    case PROP_IS_VALID:
      g_value_set_boolean (value, expidus_mime_helper_chooser_get_is_valid (chooser));
      break;
...
static void
expidus_mime_helper_chooser_set_property (GObject      *object,
                                 guint         prop_id,
                                 const GValue *value,
                                 GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_CATEGORY:
      expidus_mime_helper_chooser_set_category (chooser, g_value_get_enum (value));
      break;
...
static void
expidus_mime_helper_chooser_update (ExpidusMimeHelperChooser *chooser)
{
  ...
  g_return_if_fail (EXPIDUS_MIME_IS_HELPER_CHOOSER (chooser));

  /* determine the default helper for the category */
  helper = expidus_mime_helper_database_get_default (chooser->database, chooser->category);
  if (G_LIKELY (helper != NULL))
    {
      ...
      icon_name = expidus_mime_helper_get_icon (helper);
      ...
      gtk_label_set_text (GTK_LABEL (chooser->label), expidus_mime_helper_get_name (helper));
...
menu_activate (GtkWidget        *item,
               ExpidusMimeHelperChooser *chooser)
{
  ...
  g_assert (EXPIDUS_MIME_HELPER_N_CATEGORIES == G_N_ELEMENTS (CATEGORY_ERRORS));

  g_return_if_fail (GTK_IS_WIDGET (item));
  g_return_if_fail (EXPIDUS_MIME_IS_HELPER_CHOOSER (chooser));

  /* determine the helper for the item */
  helper = g_object_get_data (G_OBJECT (item), I_("endo-helper"));
  if (G_LIKELY (helper != NULL))
    {
      if (!expidus_mime_helper_database_set_default (chooser->database, chooser->category, helper, &error))
        {
      ...
      else
        {
          ...
          expidus_mime_helper_chooser_update (chooser);
        }
    }
}
                     ...
                     ExpidusMimeHelperChooser *chooser)
{
  ...
  {
    N_("Specify the application you want to use\nas default Web Browser for Expidus:"),
    N_("Specify the application you want to use\nas default Mail Reader for Expidus:"),
    N_("Specify the application you want to use\nas default File Manager for Expidus:"),
    N_("Specify the application you want to use\nas default Terminal Emulator for Expidus:"),
  };
  ...
  g_assert (EXPIDUS_MIME_HELPER_N_CATEGORIES == G_N_ELEMENTS (BROWSE_TITLES));
  g_assert (EXPIDUS_MIME_HELPER_N_CATEGORIES == G_N_ELEMENTS (BROWSE_MESSAGES));
  ...
  helper = expidus_mime_helper_database_get_custom (chooser->database, chooser->category);
  if (G_LIKELY (helper != NULL))
    {
      command = expidus_mime_helper_get_command (helper);
  ...
  if (gtk_dialog_run (GTK_DIALOG (dialog)) == GTK_RESPONSE_OK)
    {
      ...
      expidus_mime_helper_database_set_custom (chooser->database, chooser->category, command);
      ...
      if (G_LIKELY (helper != NULL))
        {
          ...
          g_object_set_data_full (G_OBJECT (item), I_("endo-helper"), helper, g_object_unref);
...
static void
expidus_mime_helper_chooser_pressed (ExpidusMimeHelperChooser *chooser,
                            GtkWidget        *button)
{
  AtkRelationSet *relations;
  AtkRelation    *relation;
  ...
  helpers = expidus_mime_helper_database_get_all (chooser->database, chooser->category);
  for (lp = helpers, icon = NULL; lp != NULL; lp = lp->next)
    {
      /* determine the helper */
      helper = EXPIDUS_MIME_HELPER (lp->data);

      /* create a menu item for the helper */
      item_label = gtk_label_new (expidus_mime_helper_get_name (helper));
...
/**
 * expidus_mime_helper_chooser_new:
 * @category : the #ExpidusMimeHelperCategory for the chooser.
 *
 * Allocates a new #ExpidusMimeHelperChooser for the given
 * @category.
 *
 * Return value: the newly allocated #ExpidusMimeHelperChooser.
 **/
GtkWidget*
expidus_mime_helper_chooser_new (ExpidusMimeHelperCategory category)
{
  g_return_val_if_fail (category < EXPIDUS_MIME_HELPER_N_CATEGORIES, NULL);
  return g_object_new (EXPIDUS_MIME_TYPE_HELPER_CHOOSER, "category", category, NULL);
}



/**
 * expidus_mime_helper_chooser_get_category:
 * @chooser : a #ExpidusMimeHelperChooser.
 *
 * Returns the #ExpidusMimeHelperCategory which is configured
 * by @chooser.
 *
 * Return value: the category for @chooser.
 **/
ExpidusMimeHelperCategory
expidus_mime_helper_chooser_get_category (const ExpidusMimeHelperChooser *chooser)
{
  g_return_val_if_fail (EXPIDUS_MIME_IS_HELPER_CHOOSER (chooser), EXPIDUS_MIME_HELPER_WEBBROWSER);
...
/**
 * expidus_mime_helper_chooser_set_category:
 * @chooser  : a #ExpidusMimeHelperChooser.
 ...
 **/
void
expidus_mime_helper_chooser_set_category (ExpidusMimeHelperChooser *chooser,
                                 ExpidusMimeHelperCategory category)
{
  g_return_if_fail (EXPIDUS_MIME_IS_HELPER_CHOOSER (chooser));
...
/**
 * expidus_mime_helper_chooser_get_is_valid:
 * @chooser : a #ExpidusMimeHelperChooser.
 *
 ...
 **/
gboolean
expidus_mime_helper_chooser_get_is_valid (const ExpidusMimeHelperChooser *chooser)
{
  g_return_val_if_fail (EXPIDUS_MIME_IS_HELPER_CHOOSER (chooser), FALSE);
