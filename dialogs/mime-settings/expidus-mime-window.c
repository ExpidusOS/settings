...
#include "expidus-mime-helper-chooser.h"
#include "expidus-mime-window.h"
#include "expidus-mime-chooser.h"



static void     expidus_mime_window_finalize           (GObject              *object);
static gboolean expidus_mime_window_delete_event       (GtkWidget            *widget,
                                                     GdkEventAny          *event);
static gint     expidus_mime_window_mime_model         (ExpidusMimeWindow       *window);
static void     expidus_mime_window_set_application_cb (GtkButton            *button,
                                                     ExpidusMimeWindow       *window);
static void     expidus_mime_window_filter_changed     (GtkEntry             *entry,
                                                     ExpidusMimeWindow       *window);
static void     expidus_mime_window_filter_clear       (GtkEntry             *entry,
                                                     GtkEntryIconPosition  icon_pos,
                                                     GdkEvent             *event,
                                                     gpointer              user_data);
static void     expidus_mime_window_statusbar_count    (ExpidusMimeWindow       *window,
                                                     gint                  n_mime_types);
static gboolean expidus_mime_window_row_visible_func   (GtkTreeModel         *model,
                                                     GtkTreeIter          *iter,
                                                     gpointer              data);
static void     expidus_mime_window_row_activated      (GtkTreeView          *tree_view,
                                                     GtkTreePath          *path,
                                                     GtkTreeViewColumn    *column,
                                                     ExpidusMimeWindow       *window);
static void     expidus_mime_window_selection_changed  (GtkTreeSelection     *selection,
                                                     ExpidusMimeWindow       *window);
static void     expidus_mime_window_column_clicked     (GtkTreeViewColumn    *column,
                                                     ExpidusMimeWindow       *window);
static void     expidus_mime_window_combo_populate     (GtkCellRenderer      *renderer,
...
struct _ExpidusMimeWindowClass
{
    ExpidusTitledDialogClass __parent__;
};

struct _ExpidusMimeWindow
{
...
G_DEFINE_TYPE (ExpidusMimeWindow, expidus_mime_window, EXPIDUS_TYPE_TITLED_DIALOG)



static void
expidus_mime_window_class_init (ExpidusMimeWindowClass *klass)
{
    GObjectClass   *gobject_class;
    GtkWidgetClass *gtkwidget_class;

    gobject_class = G_OBJECT_CLASS (klass);
    gobject_class->finalize = expidus_mime_window_finalize;

    gtkwidget_class = GTK_WIDGET_CLASS (klass);
    gtkwidget_class->delete_event = expidus_mime_window_delete_event;
}



static void
expidus_mime_window_init (ExpidusMimeWindow *window)
{
    GtkWidget         *vbox;
    GtkWidget         *hbox;
    GtkWidget         *button;
    ...
    g_assert (EXPIDUS_MIME_HELPER_N_CATEGORIES == 4);

    window->channel = esconf_channel_new ("expidus1-mime-settings");

    window->attrs_bold = pango_attr_list_new ();
    pango_attr_list_insert (window->attrs_bold, pango_attr_weight_new (PANGO_WEIGHT_BOLD));

    n_mime_types = expidus_mime_window_mime_model (window);

    gtk_window_set_title (GTK_WINDOW (window), _("Default Applications"));
    gtk_window_set_icon_name (GTK_WINDOW (window), "com.expidus.settings.default-applications");
    gtk_window_set_type_hint (GTK_WINDOW (window), GDK_WINDOW_TYPE_HINT_NORMAL);
    expidus_titled_dialog_create_action_area (EXPIDUS_TITLED_DIALOG (window));
    button = expidus_titled_dialog_add_button (EXPIDUS_TITLED_DIALOG (window), _("_Close"), GTK_RESPONSE_CLOSE);
    image = gtk_image_new_from_icon_name ("window-close-symbolic", GTK_ICON_SIZE_BUTTON);
    gtk_button_set_image (GTK_BUTTON (button), image);
    button = expidus_titled_dialog_add_button (EXPIDUS_TITLED_DIALOG (window), _("_Help"), GTK_RESPONSE_HELP);
    image = gtk_image_new_from_icon_name ("help-browser", GTK_ICON_SIZE_BUTTON);
    gtk_button_set_image (GTK_BUTTON (button), image);

    /* restore old user size */
    gtk_window_set_default_size (GTK_WINDOW (window),
        esconf_channel_get_int (window->channel, "/last/window-width", 550),
        esconf_channel_get_int (window->channel, "/last/window-height", 400));
    ...
    chooser = expidus_mime_helper_chooser_new (EXPIDUS_MIME_HELPER_WEBBROWSER);
    ...
    chooser = expidus_mime_helper_chooser_new (EXPIDUS_MIME_HELPER_MAILREADER);
    gtk_box_pack_start (GTK_BOX (box), chooser, FALSE, FALSE, 0);
    gtk_widget_show (chooser);
    ...
    chooser = expidus_mime_helper_chooser_new (EXPIDUS_MIME_HELPER_FILEMANAGER);
    gtk_box_pack_start (GTK_BOX (box), chooser, FALSE, FALSE, 0);
    gtk_widget_show (chooser);
    ...
    chooser = expidus_mime_helper_chooser_new (EXPIDUS_MIME_HELPER_TERMINALEMULATOR);
    gtk_box_pack_start (GTK_BOX (box), chooser, FALSE, FALSE, 0);
    gtk_widget_show (chooser);
    ...
    g_signal_connect (G_OBJECT (entry), "icon-release",
        G_CALLBACK (expidus_mime_window_filter_clear), NULL);
    g_signal_connect (G_OBJECT (entry), "changed",
        G_CALLBACK (expidus_mime_window_filter_changed), window);
    ...
    g_signal_connect (G_OBJECT (button), "clicked",
        G_CALLBACK (expidus_mime_window_set_application_cb), window);
    ...
    expidus_mime_window_statusbar_count (window, n_mime_types);
    ...
    gtk_tree_model_filter_set_visible_func (GTK_TREE_MODEL_FILTER (window->filter_model),
                                            expidus_mime_window_row_visible_func,
                                            window, NULL);
    ...
    g_signal_connect (G_OBJECT (treeview), "row-activated",
        G_CALLBACK (expidus_mime_window_row_activated), window);

    selection = gtk_tree_view_get_selection (GTK_TREE_VIEW (treeview));
    g_signal_connect (G_OBJECT (selection), "changed",
        G_CALLBACK (expidus_mime_window_selection_changed), window);
    ...
    g_signal_connect (G_OBJECT (column), "clicked",
        G_CALLBACK (expidus_mime_window_column_clicked), window);
    gtk_tree_view_append_column (GTK_TREE_VIEW (treeview), column);

    gtk_tree_view_column_set_min_width (column, 300);
    gtk_tree_view_column_set_fixed_width (column, esconf_channel_get_int (window->channel,
                                                                          "/last/mime-width",
                                                                          300));
    ...
    g_signal_connect (G_OBJECT (renderer), "editing-started",
        G_CALLBACK (expidus_mime_window_combo_populate), window);
    gtk_cell_layout_pack_start (GTK_CELL_LAYOUT (column), renderer, TRUE);
...
static void
expidus_mime_window_finalize (GObject *object)
{
    ExpidusMimeWindow *window = EXPIDUS_MIME_WINDOW (object);
    ...
    (*G_OBJECT_CLASS (expidus_mime_window_parent_class)->finalize) (object);
}



static gboolean
expidus_mime_window_delete_event (GtkWidget   *widget,
                               GdkEventAny *event)
{
    ExpidusMimeWindow    *window = EXPIDUS_MIME_WINDOW (widget);
    gint               width, height;
    GtkTreeViewColumn *column;
    guint              i;
    ...
    g_return_val_if_fail (ESCONF_IS_CHANNEL (window->channel), FALSE);
    ...
    if ((state & (GDK_WINDOW_STATE_MAXIMIZED | GDK_WINDOW_STATE_FULLSCREEN)) == 0)
    {
        ...
        esconf_channel_set_int (window->channel, "/last/window-width", width),
        esconf_channel_set_int (window->channel, "/last/window-height", height);
        ...
        for (i = 0; i < G_N_ELEMENTS (columns); i++)
        {
            ...
            esconf_channel_set_int (window->channel, prop,
                                    gtk_tree_view_column_get_width (column));
        }
    }

    if (GTK_WIDGET_CLASS (expidus_mime_window_parent_class)->delete_event != NULL)
        return (*GTK_WIDGET_CLASS (expidus_mime_window_parent_class)->delete_event) (widget, event);
    else
        return FALSE;
}
...
static GHashTable *
expidus_mime_window_mime_user (void)
{
    gchar       *filename;
    GHashTable  *table;
    ExpidusRc      *rc;
    ...
    filename = expidus_resource_save_location (EXPIDUS_RESOURCE_CONFIG, "mimeapps.list", FALSE);
    if (filename == NULL)
    {
        /* deprecated location (glib < 2.41) */
        filename = expidus_resource_save_location (EXPIDUS_RESOURCE_DATA, "applications/mimeapps.list", FALSE);
        if (filename == NULL)
            return table;
    }
    ...
    if (G_LIKELY (rc != NULL))
    {
        ...
        {
            mimes = expidus_rc_get_entries (rc, groups[n]);
...
static gint
expidus_mime_window_mime_model (ExpidusMimeWindow *window)
{
    GtkListStore *model;
    GList        *mime_types, *li;
    gchar        *mime_type;
    ...
    user_mime = expidus_mime_window_mime_user ();
...
static void
expidus_mime_window_set_application_cb (GtkButton      *button,
                                     ExpidusMimeWindow *window)
{
    GtkTreeSelection *selection;
    GtkTreeModel *model;
    ...
    expidus_mime_window_row_activated (GTK_TREE_VIEW (window->treeview), path, NULL, window);
}



static void
expidus_mime_window_filter_changed (GtkEntry       *entry,
                                 ExpidusMimeWindow *window)
{
    const gchar *text;
    gint         count;
    ...
    expidus_mime_window_statusbar_count (window, count);
}



static void
expidus_mime_window_filter_clear (GtkEntry            *entry,
                               GtkEntryIconPosition icon_pos,
                               GdkEvent            *event,
                               gpointer             user_data)
{
...
static void
expidus_mime_window_statusbar_count (ExpidusMimeWindow *window,
                                  gint            n_mime_types)
{
    gchar *msg;
...
static gboolean
expidus_mime_window_row_visible_func (GtkTreeModel *model,
                                   GtkTreeIter  *iter,
                                   gpointer      data)
{
    ExpidusMimeWindow *window = EXPIDUS_MIME_WINDOW (data);
...
static void
expidus_mime_window_set_filter_model (ExpidusMimeWindow *window,
...
static void
expidus_mime_window_set_default_for_type (ExpidusMimeWindow *window,
                                       GAppInfo       *app_info,
                                       const gchar    *mime_type,
                                       GtkTreePath    *filter_path)
{
    ...
    g_return_if_fail (EXPIDUS_IS_MIME_WINDOW (window));
        ...
        || !g_app_info_equal (app_default, app_info))
    {
        ...
        {
            expidus_mime_window_set_filter_model (window, filter_path,
                                               g_app_info_get_name (app_info), TRUE);
        }
        else
        {
            expidus_dialog_show_error (GTK_WINDOW (window), error,
...
static void
expidus_mime_window_row_activated (GtkTreeView       *tree_view,
                                GtkTreePath       *path,
                                GtkTreeViewColumn *column,
                                ExpidusMimeWindow    *window)
{
    ...
    if (gtk_tree_model_get_iter (window->filter_model, &iter, path))
    {
        ...
        dialog = g_object_new (EXPIDUS_TYPE_MIME_CHOOSER, NULL);
        gtk_window_set_transient_for (GTK_WINDOW (dialog), GTK_WINDOW (window));
        expidus_mime_chooser_set_mime_type (EXPIDUS_MIME_CHOOSER (dialog), mime_type);

        if (gtk_dialog_run (GTK_DIALOG (dialog)) == GTK_RESPONSE_YES)
        {
            app_info = expidus_mime_chooser_get_app_info (EXPIDUS_MIME_CHOOSER (dialog));
            if (G_LIKELY (app_info != NULL))
            {
                expidus_mime_window_set_default_for_type (window, app_info, mime_type, path);
...
static void
expidus_mime_window_selection_changed (GtkTreeSelection *selection,
                                    ExpidusMimeWindow   *window)
{
    gchar        *mime_type;
    gchar        *description;
...
static void
expidus_mime_window_column_clicked (GtkTreeViewColumn *column,
                                 ExpidusMimeWindow    *window)
{
...
static gboolean
expidus_mime_window_combo_row_separator_func (GtkTreeModel *model,
                                           GtkTreeIter  *iter,
                                           gpointer      data)
{
    guint type;
...
static void
expidus_mime_window_combo_unref_data (gpointer user_data)
{
...
static void
expidus_mime_window_chooser_response (GtkWidget       *chooser,
                                   gint             response_id,
                                   MimeChangedData *data)
{
    ...
    {
        app_info = expidus_mime_chooser_get_app_info (EXPIDUS_MIME_CHOOSER (chooser));
        if (G_LIKELY (app_info != NULL))
        {
            expidus_mime_window_set_default_for_type (data->window, app_info,
                                                   data->mime_type,
                                                   data->filter_path);
            g_object_unref (G_OBJECT (app_info));
        }
    ...
    expidus_mime_window_combo_unref_data (data);
...
static void
expidus_mime_window_reset_response (GtkWidget       *dialog,
                                 gint             response_id,
                                 MimeChangedData *data)
{
    GAppInfo    *app_default;
    ...
    if (response_id == GTK_RESPONSE_YES)
    {
        ...
        expidus_mime_window_set_filter_model (data->window, data->filter_path, app_name, FALSE);
...
static void
expidus_mime_window_combo_changed (GtkWidget       *combo,
                                MimeChangedData *data)
{
    ExpidusMimeWindow *window = EXPIDUS_MIME_WINDOW (data->window);
    GtkTreeModel   *model;
    GtkTreeIter     iter;
    guint           type;
    GAppInfo       *app_info;
    ...
    else if (type == APP_TYPE_CHOOSER)
    {
        dialog = g_object_new (EXPIDUS_TYPE_MIME_CHOOSER, NULL);
        gtk_window_set_transient_for (GTK_WINDOW (dialog), GTK_WINDOW (window));
        expidus_mime_chooser_set_mime_type (EXPIDUS_MIME_CHOOSER (dialog), data->mime_type);

        /* ref data */
        data->ref_count++;

        g_signal_connect (G_OBJECT (dialog), "response",
            G_CALLBACK (expidus_mime_window_chooser_response), data);
    ...
    else if (type == APP_TYPE_RESET)
    {
        ...
        dialog = expidus_message_dialog_new (GTK_WINDOW (window),
                                          _("Question"),
                                          "dialog-question",
                                          primary,
                                          ...
                                          EXPIDUS_BUTTON_TYPE_MIXED, "document-revert",
        ...
        g_signal_connect (G_OBJECT (dialog), "response",
            G_CALLBACK (expidus_mime_window_reset_response), data);
        gtk_window_present (GTK_WINDOW (dialog));
    }
}
...
static void
expidus_mime_window_combo_populate (GtkCellRenderer *renderer,
                                 GtkCellEditable *editable,
                                 const gchar     *path_string,
                                 ExpidusMimeWindow  *window)
{
    ...
    g_signal_connect_data (G_OBJECT (editable), "changed",
        G_CALLBACK (expidus_mime_window_combo_changed), data,
        (GClosureNotify) (void (*)(void)) expidus_mime_window_combo_unref_data, 0);
    gtk_combo_box_set_row_separator_func (GTK_COMBO_BOX (editable),
        expidus_mime_window_combo_row_separator_func, NULL, NULL);
...
ExpidusMimeWindow *
expidus_mime_window_new (void)
{
  return g_object_new (EXPIDUS_TYPE_MIME_WINDOW, NULL);
}



GtkWidget *
expidus_mime_window_create_dialog (ExpidusMimeWindow *window)
{
  g_return_val_if_fail (EXPIDUS_IS_MIME_WINDOW (window), NULL);
...
GtkWidget *
expidus_mime_window_create_plug (ExpidusMimeWindow *window,
                              gint            socket_id)
{
  GtkWidget *plug;
  GObject   *child;
