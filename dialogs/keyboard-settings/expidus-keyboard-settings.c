 ...
 *               2008 Olivier Fourdan <olivier@expidus.org>
...
typedef struct _ExpidusKeyboardShortcutInfo    ExpidusKeyboardShortcutInfo;

typedef
void (*ExpidusKeyboardLayoutsComboInitFunc) (ExpidusKeyboardSettings *settings);

typedef
void (*ExpidusKeyboardLayoutsComboChangedFunc) (GtkComboBox          *combo,
                                             ExpidusKeyboardSettings *settings);


static void                      expidus_keyboard_settings_constructed           (GObject                   *object);
static void                      expidus_keyboard_settings_finalize              (GObject                   *object);
static void                      expidus_keyboard_settings_row_activated         (GtkTreeView               *tree_view,
                                                                               GtkTreePath               *path,
                                                                               GtkTreeViewColumn         *column,
                                                                               ExpidusKeyboardSettings      *settings);
static void                      expidus_keyboard_settings_initialize_shortcuts  (ExpidusKeyboardSettings      *settings);
static void                      expidus_keyboard_settings_load_shortcuts        (ExpidusKeyboardSettings      *settings);
static void                      expidus_keyboard_settings_edit_shortcut         (ExpidusKeyboardSettings      *settings,
                                                                               GtkTreeView               *tree_view,
                                                                               GtkTreePath               *path);
static void                      expidus_keyboard_settings_edit_command          (ExpidusKeyboardSettings      *settings,
                                                                               GtkTreeView               *tree_view,
                                                                               GtkTreePath               *path);
static gboolean                  expidus_keyboard_settings_validate_shortcut     (ExpidusShortcutDialog        *dialog,
                                                                               const gchar               *shortcut,
                                                                               ExpidusKeyboardSettings      *settings);
static ExpidusKeyboardShortcutInfo *expidus_keyboard_settings_get_shortcut_info     (ExpidusKeyboardSettings      *settings,
                                                                               const gchar               *shortcut);
static void                      expidus_keyboard_settings_free_shortcut_info    (ExpidusKeyboardShortcutInfo  *info);
static void                      expidus_keyboard_settings_shortcut_added        (ExpidusShortcutsProvider     *provider,
                                                                               const gchar               *shortcut,
                                                                               ExpidusKeyboardSettings      *settings);
static void                      expidus_keyboard_settings_shortcut_removed      (ExpidusShortcutsProvider     *provider,
                                                                               const gchar               *shortcut,
                                                                               ExpidusKeyboardSettings      *settings);
static void                      expidus_keyboard_settings_add_button_clicked    (ExpidusKeyboardSettings      *settings,
                                                                               GtkButton                 *button);
static void                      expidus_keyboard_settings_edit_button_clicked   (ExpidusKeyboardSettings      *settings);
static void                      expidus_keyboard_settings_delete_button_clicked (ExpidusKeyboardSettings      *settings);
static void                      expidus_keyboard_settings_reset_button_clicked  (ExpidusKeyboardSettings      *settings);

#ifdef HAVE_LIBXKLAVIER

static gboolean                  expidus_keyboard_settings_update_sensitive      (GtkSwitch                 *widget,
                                                                               ExpidusKeyboardSettings      *settings);
static void                      expidus_keyboard_settings_system_default_cb     (GtkSwitch                 *widget,
                                                                               gboolean                   state,
                                                                               ExpidusKeyboardSettings      *settings);
static void                      expidus_keyboard_settings_set_layout            (ExpidusKeyboardSettings      *settings);
static void                      expidus_keyboard_settings_init_layout           (ExpidusKeyboardSettings      *settings);

static void                      expidus_keyboard_settings_layouts_combo_populate(ExpidusKeyboardSettings     *settings,
                                                                               const gchar              *combo_name,
                                                                               const gchar              *option_group_name,
                                                                               ExpidusKeyboardLayoutsComboInitFunc init_func,
                                                                               ExpidusKeyboardLayoutsComboChangedFunc cb_func);
static void                      expidus_keyboard_settings_layouts_combo_init    (ExpidusKeyboardSettings      *settings,
                                                                               const gchar               *combo_name,
                                                                               const gchar               *esconf_prop_name,
                                                                               const gchar               *default_value);
static void                      expidus_keyboard_settings_layouts_combo_add     (XklConfigRegistry         *config_registry,
                                                                               const XklConfigItem       *config_item,
                                                                               gpointer                   user_data);
static void                      expidus_keyboard_settings_layouts_combo_changed (GtkComboBox               *combo,
                                                                               ExpidusKeyboardSettings      *settings,
                                                                               const gchar               *esconf_prop_name);

static void                      expidus_keyboard_settings_init_model            (ExpidusKeyboardSettings      *settings);
static void                      expidus_keyboard_settings_model_changed_cb      (GtkComboBox               *combo,
                                                                               ExpidusKeyboardSettings      *settings);

static void                      expidus_keyboard_settings_init_grpkey           (ExpidusKeyboardSettings      *settings);
static void                      expidus_keyboard_settings_grpkey_changed_cb     (GtkComboBox               *combo,
                                                                               ExpidusKeyboardSettings      *settings);

static void                      expidus_keyboard_settings_init_compkey          (ExpidusKeyboardSettings      *settings);
static void                      expidus_keyboard_settings_compkey_changed_cb    (GtkComboBox               *combo,
                                                                               ExpidusKeyboardSettings      *settings);

static void                      expidus_keyboard_settings_update_layout_buttons (ExpidusKeyboardSettings      *settings);
static void                      expidus_keyboard_settings_edit_layout_button_cb (GtkWidget                 *widget,
                                                                               ExpidusKeyboardSettings      *settings);
static void                      expidus_keyboard_settings_add_layout_button_cb  (GtkWidget                 *widget,
                                                                               ExpidusKeyboardSettings      *settings);
static void                      expidus_keyboard_settings_row_activated_cb      (GtkTreeView               *tree_view,
                                                                               GtkTreePath               *path,
                                                                               GtkTreeViewColumn         *column,
                                                                               ExpidusKeyboardSettings      *settings);
static void                      expidus_keyboard_settings_del_layout_button_cb  (GtkWidget                 *widget,
                                                                               ExpidusKeyboardSettings      *settings);
static void                      expidus_keyboard_settings_layout_move           (GtkWidget                 *widget,
                                                                               ExpidusKeyboardSettings      *settings,
                                                                               MOVE_LAYOUT_DIRECTION      direction);
static void                      expidus_keyboard_settings_up_layout_button_cb   (GtkWidget                 *widget,
                                                                               ExpidusKeyboardSettings      *settings);
static void                      expidus_keyboard_settings_down_layout_button_cb (GtkWidget                 *widget,
                                                                               ExpidusKeyboardSettings      *settings);
static void                      expidus_keyboard_settings_add_variant_to_list   (XklConfigRegistry         *config_registry,
                                                                               XklConfigItem             *config_item,
                                                                               ExpidusKeyboardSettings      *settings);
static void                      expidus_keyboard_settings_add_layout_to_list    (XklConfigRegistry         *config_registry,
                                                                               XklConfigItem             *config_item,
                                                                               ExpidusKeyboardSettings      *settings);
static gchar**                   expidus_keyboard_settings_layout_selection      (ExpidusKeyboardSettings      *settings,
                                                                               const gchar               *layout,
                                                                               const gchar               *variant);
...
struct _ExpidusKeyboardSettingsPrivate
{
  ...
  EsconfChannel         *keyboards_channel;
  EsconfChannel         *keyboard_layout_channel;
  EsconfChannel         *xsettings_channel;
};

struct _ExpidusKeyboardShortcutInfo
{
  ExpidusShortcutsProvider *provider;
...
G_DEFINE_TYPE_WITH_PRIVATE (ExpidusKeyboardSettings, expidus_keyboard_settings, GTK_TYPE_BUILDER)



static void
expidus_keyboard_settings_class_init (ExpidusKeyboardSettingsClass *klass)
{
  GObjectClass *gobject_class;

  gobject_class = G_OBJECT_CLASS (klass);
  gobject_class->constructed = expidus_keyboard_settings_constructed;
  gobject_class->finalize = expidus_keyboard_settings_finalize;
}



static void
expidus_keyboard_settings_init (ExpidusKeyboardSettings *settings)
{
  GError *error = NULL;

  settings->priv = expidus_keyboard_settings_get_instance_private (settings);

  settings->priv->keyboards_channel = esconf_channel_new ("keyboards");
  settings->priv->keyboard_layout_channel = esconf_channel_new ("keyboard-layout");
  settings->priv->xsettings_channel = esconf_channel_new ("xsettings");

  settings->priv->provider = expidus_shortcuts_provider_new ("commands");
  g_signal_connect (settings->priv->provider, "shortcut-added",
                    G_CALLBACK (expidus_keyboard_settings_shortcut_added), settings);
  g_signal_connect (settings->priv->provider, "shortcut-removed",
                    G_CALLBACK (expidus_keyboard_settings_shortcut_removed), settings);
...
#ifdef HAVE_LIBXKLAVIER
static void
expidus_keyboard_settings_layouts_combo_populate (ExpidusKeyboardSettings *settings,
                                               const gchar *combo_name,
                                               const gchar *option_group_name,
                                               ExpidusKeyboardLayoutsComboInitFunc combo_init_func,
                                               ExpidusKeyboardLayoutsComboChangedFunc combo_changed_func)
{
  ...
  if (option_group_name != NULL)
  {
    xkl_config_registry_foreach_option (settings->priv->xkl_registry,
                                        option_group_name,
                                        expidus_keyboard_settings_layouts_combo_add,
                                        list_store);
  }
  else
  {
...
static void
expidus_keyboard_settings_constructed (GObject *object)
{
  ExpidusKeyboardSettings *settings = EXPIDUS_KEYBOARD_SETTINGS (object);
  GtkTreeViewColumn    *column;
  GtkCellRenderer      *renderer;
  ...
  esconf_g_property_bind (settings->priv->keyboards_channel, "/Default/KeyRepeat", G_TYPE_BOOLEAN, G_OBJECT (xkb_key_repeat_check), "active");

  xkb_key_repeat_rate = gtk_builder_get_object (GTK_BUILDER (settings), "xkb_key_repeat_rate");
  esconf_g_property_bind (settings->priv->keyboards_channel, "/Default/KeyRepeat/Rate", G_TYPE_INT, xkb_key_repeat_rate, "value");

  xkb_key_repeat_delay = gtk_builder_get_object (GTK_BUILDER (settings), "xkb_key_repeat_delay");
  esconf_g_property_bind (settings->priv->keyboards_channel, "/Default/KeyRepeat/Delay", G_TYPE_INT, xkb_key_repeat_delay, "value");

  xkb_numlock = gtk_builder_get_object (GTK_BUILDER (settings), "restore_numlock");
  esconf_g_property_bind (settings->priv->keyboards_channel, "/Default/RestoreNumlock", G_TYPE_BOOLEAN, xkb_numlock, "active");
  ...
  g_object_bind_property(G_OBJECT (net_cursor_blink_check), "active",
                         ...
                         G_BINDING_SYNC_CREATE | G_BINDING_BIDIRECTIONAL);
  esconf_g_property_bind (settings->priv->xsettings_channel, "/Net/CursorBlink", G_TYPE_BOOLEAN, G_OBJECT (net_cursor_blink_check), "active");

  net_cursor_blink_time = gtk_builder_get_object (GTK_BUILDER (settings), "net_cursor_blink_time");
  esconf_g_property_bind (settings->priv->xsettings_channel, "/Net/CursorBlinkTime", G_TYPE_INT, net_cursor_blink_time, "value");
  ...
  g_signal_connect (kbd_shortcuts_view, "row-activated", G_CALLBACK (expidus_keyboard_settings_row_activated), settings);
  ...
  g_signal_connect_swapped (button, "clicked", G_CALLBACK (expidus_keyboard_settings_add_button_clicked), settings);

  /* Connect to edit button */
  button = gtk_builder_get_object (GTK_BUILDER (settings), "edit_shortcut_button");
  g_signal_connect_swapped (button, "clicked", G_CALLBACK (expidus_keyboard_settings_edit_button_clicked), settings);

  /* Connect to remove button */
  button = gtk_builder_get_object (GTK_BUILDER (settings), "delete_shortcut_button");
  g_signal_connect_swapped (button, "clicked", G_CALLBACK (expidus_keyboard_settings_delete_button_clicked), settings);

  /* Connect to reset button */
  button = gtk_builder_get_object (GTK_BUILDER (settings), "reset_shortcuts_button");
  g_signal_connect_swapped (button, "clicked", G_CALLBACK (expidus_keyboard_settings_reset_button_clicked), settings);

  expidus_keyboard_settings_initialize_shortcuts (settings);
  expidus_keyboard_settings_load_shortcuts (settings);
  ...
  esconf_g_property_bind (settings->priv->keyboard_layout_channel, "/Default/XkbDisable", G_TYPE_BOOLEAN,
                          (GObject *) xkb_use_system_default_switch, "active");
  g_signal_connect (G_OBJECT (xkb_use_system_default_switch),
                    "state-set",
                    G_CALLBACK (expidus_keyboard_settings_system_default_cb),
                    settings);
  expidus_keyboard_settings_update_sensitive (GTK_SWITCH (xkb_use_system_default_switch), settings);

  /* Keyboard model combo */
  expidus_keyboard_settings_layouts_combo_populate (settings,
                                                 "xkb_model_combo",
                                                 NULL,
                                                 expidus_keyboard_settings_init_model,
                                                 expidus_keyboard_settings_model_changed_cb);
  /* Group key combo */
  expidus_keyboard_settings_layouts_combo_populate (settings,
                                                 "xkb_grpkey_combo",
                                                 "grp",
                                                 expidus_keyboard_settings_init_grpkey,
                                                 expidus_keyboard_settings_grpkey_changed_cb);
  /* Compose key combo */
  expidus_keyboard_settings_layouts_combo_populate (settings,
                                                 "xkb_composekey_combo",
                                                 "Compose key",
                                                 expidus_keyboard_settings_init_compkey,
                                                 expidus_keyboard_settings_compkey_changed_cb);
  ...
  expidus_keyboard_settings_init_layout (settings);
  g_signal_connect (G_OBJECT (xkb_layout_view), "row-activated", G_CALLBACK (expidus_keyboard_settings_row_activated_cb), settings);
  ...
  g_signal_connect (G_OBJECT (xkb_layout_add_button),    "clicked", G_CALLBACK (expidus_keyboard_settings_add_layout_button_cb), settings);
  g_signal_connect (G_OBJECT (xkb_layout_edit_button),   "clicked", G_CALLBACK (expidus_keyboard_settings_edit_layout_button_cb), settings);
  g_signal_connect (G_OBJECT (xkb_layout_delete_button), "clicked", G_CALLBACK (expidus_keyboard_settings_del_layout_button_cb), settings);
  g_signal_connect (G_OBJECT (xkb_layout_up_button),     "clicked", G_CALLBACK (expidus_keyboard_settings_up_layout_button_cb), settings);
  g_signal_connect (G_OBJECT (xkb_layout_down_button),   "clicked", G_CALLBACK (expidus_keyboard_settings_down_layout_button_cb), settings);

  expidus_keyboard_settings_update_layout_buttons (settings);
#endif /* HAVE_LIBXKLAVIER */
}
...
static void
expidus_keyboard_settings_finalize (GObject *object)
{
  ExpidusKeyboardSettings *settings = EXPIDUS_KEYBOARD_SETTINGS (object);
  ...
  (*G_OBJECT_CLASS (expidus_keyboard_settings_parent_class)->finalize) (object);
}



ExpidusKeyboardSettings *
expidus_keyboard_settings_new (void)
{
  return g_object_new (EXPIDUS_TYPE_KEYBOARD_SETTINGS, NULL);
}



GtkWidget *
expidus_keyboard_settings_create_dialog (ExpidusKeyboardSettings *settings)
{
  g_return_val_if_fail (EXPIDUS_IS_KEYBOARD_SETTINGS (settings), NULL);
...
GtkWidget *
expidus_keyboard_settings_create_plug (ExpidusKeyboardSettings *settings,
                                    gint                  socket_id)
{
  GtkWidget *plug;
  GObject   *child;
  ...
  expidus_widget_reparent (GTK_WIDGET (child), plug);
...
static void
expidus_keyboard_settings_row_activated (GtkTreeView          *tree_view,
                                      GtkTreePath          *path,
                                      GtkTreeViewColumn    *column,
                                      ExpidusKeyboardSettings *settings)
{
  if (column == gtk_tree_view_get_column (tree_view, SHORTCUT_COLUMN))
    expidus_keyboard_settings_edit_shortcut (settings, tree_view, path);
  else
    expidus_keyboard_settings_edit_command (settings, tree_view, path);
}



static void
expidus_keyboard_settings_initialize_shortcuts (ExpidusKeyboardSettings *settings)
{
  g_return_if_fail (EXPIDUS_IS_KEYBOARD_SETTINGS (settings));
}



static void
_expidus_keyboard_settings_load_shortcut (ExpidusShortcut         *shortcut,
                                       ExpidusKeyboardSettings *settings)
{
...
static void
expidus_keyboard_settings_load_shortcuts (ExpidusKeyboardSettings *settings)
{
  GList *shortcuts;
  ...
  g_list_foreach (shortcuts, (GFunc) _expidus_keyboard_settings_load_shortcut, settings);
...
static void
expidus_keyboard_settings_edit_shortcut (ExpidusKeyboardSettings *settings,
                                      GtkTreeView          *tree_view,
                                      GtkTreePath          *path)
{
  GtkTreeModel *model;
  ...
  if (G_LIKELY (gtk_tree_model_get_iter (model, &iter, path)))
    {
      ...
      dialog = expidus_shortcut_dialog_new ("commands", command, command);
      g_signal_connect (dialog, "validate-shortcut", G_CALLBACK (expidus_keyboard_settings_validate_shortcut), settings);
      ...
      if (G_LIKELY (response == GTK_RESPONSE_OK))
        {
          ...
          expidus_shortcuts_provider_set_shortcut (settings->priv->provider, new_shortcut, command, snotify);
        }
...
static void
expidus_keyboard_settings_edit_command (ExpidusKeyboardSettings *settings,
                                     GtkTreeView          *tree_view,
                                     GtkTreePath          *path)
{
  GtkTreeModel *model;
      ...
      if (G_LIKELY (response == GTK_RESPONSE_OK))
        {
          ...
          if (g_strcmp0 (command, new_command) != 0
              || snotify != new_snotify)
            {
              ...
              /* Save settings */
              expidus_shortcuts_provider_set_shortcut (settings->priv->provider, shortcut,
...
static gboolean
expidus_keyboard_settings_validate_shortcut (ExpidusShortcutDialog   *dialog,
                                          const gchar          *shortcut,
                                          ExpidusKeyboardSettings *settings)
{
  ExpidusKeyboardShortcutInfo *info;
  gboolean                  accepted = TRUE;
  gint                      response;

  g_return_val_if_fail (EXPIDUS_IS_SHORTCUT_DIALOG (dialog), FALSE);
  g_return_val_if_fail (EXPIDUS_IS_KEYBOARD_SETTINGS (settings), FALSE);
  ...
  info = expidus_keyboard_settings_get_shortcut_info (settings, shortcut);
  ...
  if (G_UNLIKELY (info != NULL))
    {
      response = expidus_shortcut_conflict_dialog (GTK_WINDOW (dialog),
                                                ...
                                                expidus_shortcuts_provider_get_name (info->provider),
                                                shortcut,
                                                expidus_shortcut_dialog_get_action_name (dialog),
      ...
      if (G_UNLIKELY (response == GTK_RESPONSE_ACCEPT))
        ...
        DBG ("We want to use %s with %s", shortcut,
             expidus_shortcut_dialog_get_action_name (dialog));
        expidus_shortcuts_provider_reset_shortcut (info->provider, shortcut);

        /*Remove the shortcut manually from the treeview */
        expidus_keyboard_settings_shortcut_removed (settings->priv->provider,
                                                 shortcut,
                                                 settings);
        }
      else
      ...
      expidus_keyboard_settings_free_shortcut_info (info);
    }
...
static ExpidusKeyboardShortcutInfo *
expidus_keyboard_settings_get_shortcut_info (ExpidusKeyboardSettings *settings,
                                          const gchar          *shortcut)
{
  ExpidusKeyboardShortcutInfo *info = NULL;
  GList                    *iter;
  ExpidusShortcut             *sc;
  GList                    *providers;
  ...
  for (iter = providers; iter != NULL && info == NULL; iter = g_list_next (iter))
    {
      if (G_UNLIKELY (expidus_shortcuts_provider_has_shortcut (iter->data, shortcut)))
        {
          sc = expidus_shortcuts_provider_get_shortcut (iter->data, shortcut);

          if (G_LIKELY (sc != NULL))
            {
              info = g_new0 (ExpidusKeyboardShortcutInfo, 1);
...
static void
expidus_keyboard_settings_free_shortcut_info (ExpidusKeyboardShortcutInfo *info)
{
  g_object_unref (info->provider);
  expidus_shortcut_free (info->shortcut);
  g_free (info);
}
...
static void
expidus_keyboard_settings_shortcut_added (ExpidusShortcutsProvider *provider,
                                       const gchar           *shortcut,
                                       ExpidusKeyboardSettings  *settings)
{
  ExpidusShortcut *sc;
  ...
  sc = expidus_shortcuts_provider_get_shortcut (settings->priv->provider, shortcut);
...
static gboolean
_expidus_keyboard_settings_remove_shortcut (GtkTreeModel *model,
                                         GtkTreePath  *path,
                                         GtkTreeIter  *iter,
                                         const gchar  *shortcut)
{
...
static void
expidus_keyboard_settings_shortcut_removed (ExpidusShortcutsProvider *provider,
                                         const gchar           *shortcut,
                                         ExpidusKeyboardSettings  *settings)
{
  GtkTreeModel *model;
  ...
  gtk_tree_model_foreach (model, (GtkTreeModelForeachFunc) _expidus_keyboard_settings_remove_shortcut,
                          (gpointer) shortcut);
}
...
static void
expidus_keyboard_settings_add_button_clicked (ExpidusKeyboardSettings *settings,
                                           GtkButton            *button)
{
  ...
  if (G_UNLIKELY (response == GTK_RESPONSE_OK))
    {
      ...
      shortcut_dialog = expidus_shortcut_dialog_new ("commands", command, command);
      g_signal_connect (shortcut_dialog, "validate-shortcut", G_CALLBACK (expidus_keyboard_settings_validate_shortcut), settings);
      ...
      response = expidus_shortcut_dialog_run (EXPIDUS_SHORTCUT_DIALOG (shortcut_dialog), GTK_WIDGET (parent));

      /* Only continue if the shortcut dialog succeeded */
      if (G_LIKELY (response == GTK_RESPONSE_OK))
        {
          /* Get shortcut */
          shortcut = expidus_shortcut_dialog_get_shortcut (EXPIDUS_SHORTCUT_DIALOG (shortcut_dialog));

          /* Save the new shortcut to esconf */
          DBG ("Save shortcut %s with command %s to Esconf", shortcut, command);
          expidus_shortcuts_provider_set_shortcut (settings->priv->provider, shortcut, command, snotify);
        }
...
static void
expidus_keyboard_settings_edit_button_clicked (ExpidusKeyboardSettings *settings)
{
          ...
          if (G_UNLIKELY (response == GTK_RESPONSE_OK))
            {
              ...
              /* Create shortcut dialog */
              shortcut_dialog =
                expidus_shortcut_dialog_new ("commands",
                                          new_command,
                                          new_command);

              g_signal_connect (shortcut_dialog, "validate-shortcut",
                                G_CALLBACK (expidus_keyboard_settings_validate_shortcut),
                                settings);
              ...
              response =
                expidus_shortcut_dialog_run (EXPIDUS_SHORTCUT_DIALOG (shortcut_dialog),
                                          GTK_WIDGET (parent));
              ...
              if (G_LIKELY (response == GTK_RESPONSE_OK))
                {
                  ...
                  /* Get shortcut */
                  new_shortcut =
                    expidus_shortcut_dialog_get_shortcut (EXPIDUS_SHORTCUT_DIALOG (shortcut_dialog));
                      ...
                      if (test_new_shortcut)
                        {
                          ...
                          /* Remove old keyboard shortcut via esconf */
                          expidus_shortcuts_provider_reset_shortcut (settings->priv->provider,
                                                                  shortcut);
                        }

                      /* Save settings */
                      expidus_shortcuts_provider_set_shortcut (settings->priv->provider,
                                                            new_shortcut,
                                                            new_command,
                                                            new_snotify);
                    }
...
static void
expidus_keyboard_settings_delete_button_clicked (ExpidusKeyboardSettings *settings)
{
  GtkTreeSelection *selection;
  GtkTreeModel     *model;
  GtkTreePath      *path;
...
static void
expidus_keyboard_settings_reset_button_clicked (ExpidusKeyboardSettings *settings)
{
  gint          response;
  GObject      *view;
  GtkListStore *store;
  ...
  response = expidus_message_dialog (NULL, _("Reset to Defaults"), "dialog-question",
                                  _("Reset to Defaults"),
...
static gchar *
expidus_keyboard_settings_xkb_description (XklConfigItem *config_item)
{
...
static gboolean
expidus_keyboard_settings_update_sensitive (GtkSwitch *widget, ExpidusKeyboardSettings *settings)
{
...
static void
expidus_keyboard_settings_system_default_cb (GtkSwitch *widget, gboolean state, ExpidusKeyboardSettings *settings)
{
  ...
  use_system_defaults = expidus_keyboard_settings_update_sensitive (widget, settings);
...
static void
expidus_keyboard_settings_set_layout (ExpidusKeyboardSettings *settings)
{
  GObject          *view;
  GtkTreeModel     *model;
  GtkTreeIter       iter;
  ...
  esconf_channel_set_string (settings->priv->keyboard_layout_channel,
...
static void
expidus_keyboard_settings_init_layout (ExpidusKeyboardSettings *settings)
{
  ...
  val_layout  = esconf_channel_get_string (settings->priv->keyboard_layout_channel, "/Default/XkbLayout",  default_layouts);
  val_variant = esconf_channel_get_string (settings->priv->keyboard_layout_channel, "/Default/XkbVariant",  default_variants);
...
static void
expidus_keyboard_settings_layouts_combo_add (XklConfigRegistry    *config_registry,
                                          const XklConfigItem  *config_item,
                                          gpointer              user_data)
{
  ...
  description = expidus_keyboard_settings_xkb_description ((XklConfigItem *) config_item);
...
static void
expidus_keyboard_settings_layouts_combo_init (ExpidusKeyboardSettings *settings,
                                           const gchar *combo_name,
                                           const gchar *esconf_prop_name,
                                           const gchar *default_value)
{
  ...
  esconf_prop_value = esconf_channel_get_string (settings->priv->keyboard_layout_channel, esconf_prop_name, default_value);
  item = gtk_tree_model_get_iter_first (model, &iter);

  if (esconf_prop_value == NULL || *esconf_prop_value == 0)
  {
  ...
  while (item && !found)
    {
      ...
      found = !strcmp (id, esconf_prop_value);
      g_free (id);
  ...
  g_free (esconf_prop_value);
}

static void
expidus_keyboard_settings_init_model (ExpidusKeyboardSettings *settings)
{
  expidus_keyboard_settings_layouts_combo_init (settings,
                                             "xkb_model_combo",
                                             "/Default/XkbModel",
...
static void
expidus_keyboard_settings_init_grpkey (ExpidusKeyboardSettings *settings)
{
  expidus_keyboard_settings_layouts_combo_init (settings,
                                             "xkb_grpkey_combo",
...
static void
expidus_keyboard_settings_init_compkey (ExpidusKeyboardSettings *settings)
{
  expidus_keyboard_settings_layouts_combo_init (settings,
                                             "xkb_composekey_combo",
...
static void
expidus_keyboard_settings_row_activated_cb (GtkTreeView          *tree_view,
                                         GtkTreePath          *path,
                                         GtkTreeViewColumn    *column,
                                         ExpidusKeyboardSettings *settings)
{
  expidus_keyboard_settings_edit_layout_button_cb (NULL, settings);
}



static void
expidus_keyboard_settings_layouts_combo_changed (GtkComboBox          *combo,
                                              ExpidusKeyboardSettings *settings,
                                              const gchar *esconf_prop_name)
{
  GtkTreeModel *model;
  ...
  if (G_LIKELY(gtk_combo_box_get_active_iter (GTK_COMBO_BOX (combo), &iter)))
  {
    ...
    gtk_tree_model_get (model, &iter, XKB_LAYOUTS_COMBO_VALUE, &esconf_prop_value, -1);
    esconf_channel_set_string (settings->priv->keyboard_layout_channel,
                               esconf_prop_name, esconf_prop_value);
    g_free (esconf_prop_value);
  }
}
...
static void
expidus_keyboard_settings_model_changed_cb (GtkComboBox          *combo,
                                         ExpidusKeyboardSettings *settings)
{
  expidus_keyboard_settings_layouts_combo_changed (combo, settings,
                                                "/Default/XkbModel");
}
...
static void
expidus_keyboard_settings_grpkey_changed_cb (GtkComboBox          *combo,
                                          ExpidusKeyboardSettings *settings)
{
  expidus_keyboard_settings_layouts_combo_changed (combo, settings,
                                                "/Default/XkbOptions/Group");
...
static void
expidus_keyboard_settings_compkey_changed_cb (GtkComboBox          *combo,
                                           ExpidusKeyboardSettings *settings)
{
  expidus_keyboard_settings_layouts_combo_changed (combo, settings,
                                                "/Default/XkbOptions/Compose");
...
static void
expidus_keyboard_settings_update_layout_buttons (ExpidusKeyboardSettings *settings)
{
  GObject      *view;
  GtkTreeModel *model;
  GObject      *object;
...
static void
expidus_keyboard_settings_edit_layout_button_cb (GtkWidget            *widget,
                                              ExpidusKeyboardSettings *settings)
{
  GtkTreeSelection *selection;
  GObject          *view;
  ...
  if (G_LIKELY (gtk_tree_selection_get_selected (selection, &model, &iter)))
  {
    gtk_tree_model_get (model, &iter,
     ...
     layout_selection =
        expidus_keyboard_settings_layout_selection (settings, current_layout, current_variant);
    if (layout_selection)
      {
        gtk_list_store_set (GTK_LIST_STORE (model), &iter,
                            ...
                            -1);
        expidus_keyboard_settings_set_layout (settings);
...
static void
expidus_keyboard_settings_add_layout_button_cb (GtkWidget            *widget,
                                             ExpidusKeyboardSettings *settings)
{
  GObject          *view;
  GtkTreeModel     *model;
  ...
  layout_selection = expidus_keyboard_settings_layout_selection (settings, NULL, NULL);
  view = gtk_builder_get_object (GTK_BUILDER (settings), "xkb_layout_view");
  if (layout_selection)
    {
      model = gtk_tree_view_get_model (GTK_TREE_VIEW (view));
...
static void
expidus_keyboard_settings_del_layout_button_cb (GtkWidget            *widget,
                                             ExpidusKeyboardSettings *settings)
{
  GObject          *view;
  GtkTreeModel     *model;
...
static void
expidus_keyboard_settings_up_layout_button_cb (GtkWidget *widget,
                                            ExpidusKeyboardSettings *settings)
{
    expidus_keyboard_settings_layout_move (widget, settings, MOVE_LAYOUT_UP);
}



static void
expidus_keyboard_settings_down_layout_button_cb (GtkWidget *widget,
                                              ExpidusKeyboardSettings *settings)
{
    expidus_keyboard_settings_layout_move (widget, settings, MOVE_LAYOUT_DOWN);
}



static void
expidus_keyboard_settings_layout_move (GtkWidget *widget,
                                    ExpidusKeyboardSettings *settings,
                                    MOVE_LAYOUT_DIRECTION direction)
{
  GObject          *view;
...
static void
expidus_keyboard_settings_add_variant_to_list (XklConfigRegistry    *config_registry,
                                            XklConfigItem        *config_item,
                                            ExpidusKeyboardSettings *settings)
{
  ...
  variant = expidus_keyboard_settings_xkb_description (config_item);
...
static void
expidus_keyboard_settings_add_layout_to_list (XklConfigRegistry    *config_registry,
                                           XklConfigItem        *config_item,
                                           ExpidusKeyboardSettings *settings)
{
  GtkTreeStore *treestore;
  ...
  layout = expidus_keyboard_settings_xkb_description (config_item);
  treeview = gtk_builder_get_object (GTK_BUILDER (settings), "layout_selection_view");
  treestore = GTK_TREE_STORE (gtk_tree_view_get_model (GTK_TREE_VIEW (treeview)));
  ...
  xkl_config_registry_foreach_layout_variant (config_registry, config_item->name,
      (ConfigItemProcessFunc)expidus_keyboard_settings_add_variant_to_list, settings);
}



static void
expidus_keyboard_settings_layout_activate_cb (GtkTreeView       *tree_view,
                                           GtkTreePath       *path,
                                           GtkTreeViewColumn *column,
                                           GtkDialog         *dialog)
{
...
static gchar**
expidus_keyboard_settings_layout_selection (ExpidusKeyboardSettings *settings,
  ...
  if (!settings->priv->layout_selection_treestore)
    {
      ...
      xkl_config_registry_foreach_layout (settings->priv->xkl_registry,
          (ConfigItemProcessFunc) expidus_keyboard_settings_add_layout_to_list, settings);
      g_signal_connect (GTK_TREE_VIEW (layout_selection_view), "row-activated", G_CALLBACK (expidus_keyboard_settings_layout_activate_cb), keyboard_layout_selection_dialog);
